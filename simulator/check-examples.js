...
function extractCurlBodies(markdown) {
    const bodies = [];
    // Тело может содержать вставки оболочки вида '"$id"': одинарная кавычка
    // закрывается, подставляется переменная, кавычка открывается снова.
    // Нежадный поиск до первой кавычки обрывал такие примеры на середине, и
    // обрывок потом не разбирался как JSON — инструмент сам придумывал себе
    // расхождения там, где документация в порядке.
    const re = /-d\s+'((?:[^']|'"[^"]*"')*)'\s*\\?\s*\n/g;
    let match;
