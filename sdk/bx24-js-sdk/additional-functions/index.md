...
- Методы [BX24.im.openMessenger](./bx24-im-open-messenger.md) и [BX24.im.openHistory](./bx24-im-open-history.md) принимают идентификатор диалога строкой: `userId`, `chatXXX` для чата или ```imol|XXXX``` для открытой линии; `BX24.im.openMessenger` дополнительно поддерживает `sgXXX` для чата группы. [BX24.im.callTo](./bx24-im-call-to.md) принимает числовой идентификатор пользователя, а [BX24.im.phoneTo](./bx24-im-phone-to.md) — телефонный номер строкой. Обратного вызова у методов `BX24.im.*` нет, о результате выполнения они не сообщают
