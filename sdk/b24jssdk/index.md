...
Для проекта на Nuxt есть отдельный модуль. Ему нужен Nuxt 4.2.2 или новее:

```bash
npx nuxi module add @bitrix24/b24jssdk-nuxt
```

Команда установит пакет и зарегистрирует модуль в `nuxt.config`. Плагин модуля работает только на клиенте и добавляет `$initializeB24Frame` — на сервере Nuxt его нет.
...
Второй аргумент фабрики — флаг режима разработки. При `true` логгер печатает все сообщения, при `false` или без аргумента — только ошибки, а `logger.info()` молча отбрасывается. Поэтому в примере выше строка «Компании» появится, только если переменная окружения `NODE_ENV` равна `development`.
...
Сохраняйте `b24OAuthParams` целиком. При обновлении SDK перезаписывает не только токены, но и `expires`, `expiresIn`, `scope`, `status`, `clientEndpoint` и `serverEndpoint`. Если сохранить только часть полей, после перезапуска приложение возьмет из хранилища устаревшие значения.

Если обновить токены не удалось, SDK выбрасывает исключение. Его класс зависит от того, как именно сорвалось обновление: при HTTP-ошибке от сервера авторизации это `RefreshTokenError`, в остальных случаях — обычная `Error`, а при автоматическом обновлении внутри вызова метода ошибка придет как `AjaxError`. Ловите исключение, не привязываясь к конкретному классу, — в любом из этих случаев приложение нужно авторизовать заново. Полное описание параметров и сценариев обновления токена — в [документации класса B24OAuth](https://bitrix-tools.github.io/b24jssdk/docs/working-with-the-rest-api/oauth).
...
Обертки отдают данные по-разному. У `call.make()` метод `getData()` возвращает объект `{ result, time }`, где `result` — результат метода, а `time` — данные о времени выполнения запроса. Полей `next` и `total` из ответа REST API в нем нет: для постраничного обхода в SDK есть списочные обертки. У `callList.make()` метод `getData()` сразу отдает массив записей — обращаться к `result` не нужно.
...
В `filter` не занимайте ключ с оператором «больше» по полю курсора — например `'>id'` при `idKey: 'id'`. Обертка записывает в этот ключ позицию следующей страницы и молча затирает ваше условие: вернутся все записи, предупреждения не будет. Остальные операторы и поля работают как обычно.
...
Ограничение в 50 команд `batch.make()` не обходит: если команд больше или массив пуст, он выбрасывает исключение еще до отправки запроса.
...
Сетевые сбои и превышение лимита SDK тоже отрабатывает сам: он повторяет запрос до трех раз с задержкой от секунды. Поэтому такой вызов возвращает ошибку не сразу, а в логах видна каждая попытка. Число попыток меняется полем `maxRetries` того же метода. Ошибки доступа и другие ошибки, которые повтор не исправит, SDK не повторяет.
...
В версии 3.0.0 удалят и методы постраничного обхода у результата `call.make()` — `hasMore()`, `isMore()`, `getTotal()`, `fetchNext()` и `getNext()`. Они связаны с форматом ответа REST API v2, при вызове ничего не предупреждают, а `getNext()` через `actions.v3` уже сейчас выбрасывает исключение. Для обхода списков используйте `callList.make()` и `fetchList.make()`.

{% endnote %}

## Обработка ошибок

Ошибки приходят на двух уровнях, и уровень зависит от кода ошибки, а не от того, дошел ли запрос:

- часть ошибок REST API SDK отдает мягко — `response.isSuccess` равен `false`, тексты ошибок отдает `response.getErrorMessages()`. Так приходят ошибка «сущность не найдена» и ошибки валидации REST API v3
- остальные ошибки REST API, сетевые сбои и ошибки самого SDK приходят исключением `SdkError` с полями `code` и `status`; у сбоев HTTP-вызова это его наследник `AjaxError`, добавляющий данные запроса в `requestInfo`. Так приходит большинство частых ошибок портала: `ACCESS_DENIED`, `NOT_FOUND`, `INVALID_REQUEST`, `expired_token`, `OVERLOAD_LIMIT`
...
try {
    ...
    // Ошибка доступа, сетевая ошибка, таймаут или ошибка самого SDK
