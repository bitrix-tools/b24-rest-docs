...
Iframe-приложение может читать и менять текст в поле ввода чата Битрикс24 через `$b24.parent.message.send`.
Встройка не обращается к элементам интерфейса Битрикс24 напрямую. Она отправляет запрос в родительское окно через `postMessage`, а Битрикс24 сам читает или меняет текст в поле ввода.

В JS SDK это часть работы с родительским окном: [$b24.parent](https://bitrix-tools.github.io/b24jssdk/reference/frame-parent.html).

Используйте этот API, когда нужно:
...
Битрикс24 принимает сообщения от iframe через `window.addEventListener('message', ...)`, далее система:

1. принимает запрос от встройки
2. проверяет, что сообщение пришло с того же Битрикс24
3. выполняет действие в интерфейсе мессенджера
4. возвращает результат во встройку

Важно: если источник сообщения не совпадает (`event.origin !== window.location.origin`), сообщение будет отклонено.
...
В `responseGet` придет текст из поля ввода. Если что-то пошло не так, придет ошибка.
...
В `responseSet` придет результат вставки. Если что-то пошло не так, придет ошибка.
