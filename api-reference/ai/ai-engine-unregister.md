...
Метод ищет сервис по `code` и привязке к текущему приложению. При вызове через вебхук поиск выполняется среди сервисов без привязки к приложению. Категория сервиса в поиске не участвует, передавать ее не нужно.
...
`тип` | **Описание** ||
|| **code***
[`string`](../data-types.md) | Символьный код удаляемого сервиса. Код должен совпадать с кодом сервиса, зарегистрированного текущим приложением или без привязки к приложению при вызове через вебхук ||
|#
...
- Python
    ...
    try:
        bitrix_response = client.ai.engine.unregister(
            code='acme_gpt',
        ).response
...
- `true` — сервис удален
- `false` — сервис не найден, не принадлежит текущему приложению, параметр `code` не передан или удаление не выполнено ||
|| **time**
