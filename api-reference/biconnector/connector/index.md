...
В зависимости от доступности эндпоинт возвращает информацию о статусе источника.

#### urlTableList {#urlTableList}

Эндпоинт `urlTableList` возвращает список доступных таблиц в формате `JSON`. Эндпоинт вернет таблицы, которые соответствуют поисковому запросу.

Вызов эндпоинта происходит:

-  при создании датасета,
-  при использовании в интерфейсе.
...
Эндпоинт `urlTableDescription` возвращает список полей конкретной таблицы в формате `JSON`.
...
## Параметры поля biconnector_rest_field_description {#description}
