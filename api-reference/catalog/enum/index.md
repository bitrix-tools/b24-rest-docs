...
Методы `catalog.enum.*` возвращают справочные перечисления Торгового каталога для использования в других методах.

> Быстрый переход: [все методы](#all-methods)

## Как начать работу

1. Определите, для какого сценария нужен справочник: округление цен или складские документы
2. Получите список допустимых значений методом `catalog.enum.*`
3. Передайте полученный код в связанный метод Торгового каталога
...
#|
|| **Метод** | **Описание** ||
|| [catalog.enum.getRoundTypes](./catalog-enum-get-round-types.md) | Возвращает список типов округления, доступных в каталоге ||
