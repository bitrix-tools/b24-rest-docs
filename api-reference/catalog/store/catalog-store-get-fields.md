...
- Python
    ...
    try:
        bitrix_response = client.catalog.store.get_fields().response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "catalog.store.getFields", nil, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("catalog.store.getFields: %w", err)
    }
