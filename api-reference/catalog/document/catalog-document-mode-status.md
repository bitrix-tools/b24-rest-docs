...
- Python
    ...
    try:
        bitrix_response = client.catalog.document.mode.status().response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "catalog.document.mode.status", nil)
    if err != nil {
    	return fmt.Errorf("catalog.document.mode.status: %w", err)
    }
