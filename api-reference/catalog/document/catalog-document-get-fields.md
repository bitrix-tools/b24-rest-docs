...
- Python
    ...
    try:
        bitrix_response = client.catalog.document.get_fields().response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "catalog.document.getFields", nil, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("catalog.document.getFields: %w", err)
    }
