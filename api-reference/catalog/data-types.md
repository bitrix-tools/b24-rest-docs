...
Типы данных Каталога описывают идентификаторы и структуры объектов в параметрах и ответах методов `catalog.*`. По справочнику можно определить назначение объекта, связь с торговым каталогом и формат его полей. Базовые типы `integer`, `string`, `object` и другие перечислены в статье [Типы данных в REST API](../data-types.md).

## Как выбрать объект

#|
|| **Объект** | **Когда использовать** | **Методы** ||
|| [`catalog_product`](#catalog_product) | Для простого товара без вариаций, который можно продавать как отдельную позицию | [catalog.product.*](./product/index.md) ||
|| [`catalog_product_sku`](#catalog_product_sku) | Для головного товара, который объединяет вариации и хранит общую информацию о них | [catalog.product.sku.*](./product/sku/index.md) ||
|| [`catalog_product_offer`](#catalog_product_offer) | Для вариации головного товара, например определенного цвета или размера | [catalog.product.offer.*](./product/offer/index.md) ||
|| [`catalog_product_service`](#catalog_product_service) | Для услуги, у которой нет материального остатка | [catalog.product.service.*](./product/service/index.md) ||
|| [`catalog_catalog`](#catalog_catalog) | Для определения информационных блоков товаров и вариаций и связи между ними | [catalog.catalog.*](./catalog/index.md) ||
|| [`catalog_price`](#catalog_price) | Для цены товара, вариации или услуги | [catalog.price.*](./price/index.md) ||
|| [`catalog_storeproduct`](#catalog_storeproduct) | Для остатка товара или вариации на конкретном складе | [catalog.storeproduct.*](./store-product/index.md) ||
|#

Полный список типов и структур приведен ниже. Если метод возвращает тип вида `catalog_product.id`, значение содержит идентификатор соответствующего объекта, а не весь объект.

## Связь товара и вариации

Поле `type` определяет роль товарного объекта. Оно доступно только для чтения:

- `1` — простой товар
- `3` — головной товар с вариациями
- `4` — вариация
- `5` — вариация без головного товара
- `6` — головной товар без вариаций
- `7` — услуга

Поле `iblockId` указывает, в каком информационном блоке находится объект. Головной товар хранится в информационном блоке товаров, а вариация — в отдельном информационном блоке вариаций. У объекта [`catalog_catalog`](#catalog_catalog) для информационного блока вариаций поле `productIblockId` содержит идентификатор информационного блока товаров, а `skuPropertyId` — идентификатор свойства связи с головным товаром.

Конкретная вариация связана с головным товаром через поле `parentId` объекта [`catalog_product_offer`](#catalog_product_offer). Например, для вариации с `parentId: 200` головным будет товар `catalog_product_sku` с `id: 200`.

Чтобы определить объекты и их связь:

1. Получите информационные блоки методом [catalog.catalog.list](./catalog/catalog-catalog-list.md)
2. Выберите объект по значению `type`
3. Для вариации сопоставьте `parentId` с `id` головного товара
4. Перейдите к таблице объекта и проверьте формат нужного поля
...
|| [`catalog_product_property.id`](#catalog_product_property) | Целочисленный идентификатор свойства товара или вариации (например, `1`). Получить идентификаторы свойств можно с помощью метода [catalog.productProperty.list](./product-property/catalog-product-property-list.md) ||
...
В примерах показаны поля, по которым можно определить тип объекта, его торговый каталог и связь вариации с головным товаром.
...
В примере `catalog_product_sku` с `id: 200` — головной товар. Объект `catalog_product_offer` с `parentId: 200` — его вариация в информационном блоке `13`. Простой товар и услуга находятся в информационном блоке `12`, но различаются значением `type`.
...
|| **fieldN**
[`any`](../data-types.md) | Значение пользовательского поля документа, где `N` — идентификатор пользовательского поля, например `field7097` ||
|#
...
|| **timestampX**
[`datetime`](../data-types.md) | Дата и время изменения свойства ||
|| **iblockId**
[`catalog_catalog.id`](#catalog_catalog) | Идентификатор торгового каталога ||
|| **name**
[`string`](../data-types.md) | Название свойства ||
|| **active**
[`char`](../data-types.md) | Признак активности. Возможные значения:
- `Y` — да
- `N` — нет ||
|| **sort**
...
|| **userType**
[`string`](../data-types.md) | Пользовательский тип свойства. Значение соответствует указанному `propertyType`.
...
|| **listType**
[`char`](../data-types.md) | Внешний вид списка ||
|| **multiple**
...
|| **fileType**
[`string`](../data-types.md) | Разрешенные расширения для свойства типа файл ||
|| **multipleCnt**
[`integer`](../data-types.md) | Число полей ввода для множественных значений ||
|| **linkIblockId**
[`catalog_catalog.id`](#catalog_catalog) | Идентификатор связанного инфоблока. 

Доступные идентификаторы можно получить методом [catalog.catalog.list](./catalog/catalog-catalog-list.md) ||
|| **withDescription**
[`char`](../data-types.md) | Признак хранения описания значения ||
|| **searchable**
[`char`](../data-types.md) | Признак участия в поиске. Возможные значения:
- `Y` — да
- `N` — нет ||
|| **filtrable**
[`char`](../data-types.md) | Признак участия в фильтрации ||
|| **isRequired**
...
|| **parentId**
[`catalog_product_sku.id`](#catalog_product_sku) | Идентификатор головного товара.

Получить идентификаторы головных товаров можно методом [catalog.product.sku.list](./product/sku/catalog-product-sku-list.md)
||
|| **iblockId**
[`catalog_catalog.id`](#catalog_catalog) | Идентификатор информационного блока торгового каталога.
