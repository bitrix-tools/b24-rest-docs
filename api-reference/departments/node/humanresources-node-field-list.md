# Получить список полей отдела humanresources.node.field.list
...
Метод `humanresources.node.field.list` возвращает список доступных полей отдела.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/humanresources.node.field.list`
...
- Python
    ...
    try:
        bitrix_response = client.humanresources.node.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "humanresources.node.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("humanresources.node.field.list: %w", err)
    }
...
```json
{
    ...
    "time": {
        ...
        "processing": 0,
        "date_start": "2026-06-02T15:11:40+03:00",
        "date_finish": "2026-06-02T15:11:40+03:00",
