...
- Python
    ...
    try:
        bitrix_response = client.humanresources.node.move(
            bitrix_id=44,
            parent_id=2,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "humanresources.node.move", b24.Params{
    	"id":       44,
    	"parentId": 2,
    })
    if err != nil {
    	return fmt.Errorf("humanresources.node.move: %w", err)
    }
