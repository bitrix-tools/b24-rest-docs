...
- Python
    ...
    try:
        bitrix_response = client.humanresources.node.field.get(
            name='name',
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "humanresources.node.field.get", b24.Params{
    	"name":   "name",
    	"select": []string{"name", "type", "title"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("humanresources.node.field.get: %w", err)
    }
