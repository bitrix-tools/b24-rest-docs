...
2. Настройте триггер приложения методами раздела [Автоматизация CRM](../index.md) для нужного объекта, стадии или статуса
...
**Объекты CRM.** В методе [crm.automation.trigger.execute](./crm-automation-trigger-execute.md) параметры `OWNER_TYPE_ID` и `OWNER_ID` определяют тип объекта CRM и конкретную запись для запуска триггера. Значение `OWNER_TYPE_ID` можно получить методом [crm.enum.ownertype](../../auxiliary/enum/crm-enum-owner-type.md). Идентификатор `OWNER_ID` получают методами того раздела CRM, который соответствует нужному объекту, например [crm.lead.\*](../../leads/index.md), [crm.deal.\*](../../deals/index.md), [crm.quote.\*](../../quote/index.md) или [crm.item.\*](../../universal/index.md).
