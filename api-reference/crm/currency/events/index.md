...
Каждое событие приходит только с идентификатором валюты в поле `FIELDS.ID`. Значения полей событие не передает: после создания или изменения валюты запросите их методом [crm.currency.get](../crm-currency-get.md) из раздела [Валюты в CRM](../index.md).
