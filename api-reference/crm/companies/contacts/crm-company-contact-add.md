...
>
> Кто может выполнять метод: пользователь с правом «Изменение» компаний и правом «Чтение» контакта, который добавляется

Метод `crm.company.contact.add` добавляет контакт к указанной компании.

Метод работает с одной привязкой и не затрагивает остальные контакты компании. Чтобы задать весь набор контактов сразу, используйте [crm.company.contact.items.set](./crm-company-contact-items-set.md). Как устроен объект привязки, описано в [обзоре раздела](./index.md).
...
||
|| **fields***
[`object`](../../../data-types.md) | Объект с информацией о контакте, который нужно привязать к компании.
...
Переданное значение не влияет на результат: добавленная привязка всегда получает `IS_PRIMARY = Y`.

Флаг относится к контакту и означает, что эта компания для него основная. Поэтому в привязке к прежней основной компании флаг сбрасывается в `N`, а компания из новой привязки попадает в поле контакта `COMPANY_ID` ||
|| **SORT**
[`integer`](../../../data-types.md) | Индекс сортировки.

Если `SORT` не передан, метод подставляет `i + 10`, где `i` — максимальный индекс сортировки среди привязок этой компании. Если привязок нет, `i` равен `0` ||
|#
