...
Метод заменяет набор целиком: контакты, которых нет в `items`, отвязываются от компании. У каждого отвязанного контакта поле `COMPANY_ID` переключается на другую его компанию с наименьшим идентификатором или очищается, когда у контакта нет других компаний.

Чтобы добавить или убрать один контакт, не затрагивая остальные, используйте [crm.company.contact.add](./crm-company-contact-add.md) и [crm.company.contact.delete](./crm-company-contact-delete.md). Как устроен объект привязки, описано в [обзоре раздела](./index.md).
...
||
|| **items***
[`object[]`](../../../data-types.md) | Набор объектов, которые описывают контакты, привязанные к компании. Структуру объекта привязки смотрите [ниже](#company_contact_binding).

Пустой массив отвязывает от компании все контакты.

Элементы без `CONTACT_ID` или со значением меньше либо равным `0` метод пропускает без ошибки, но их позиция учитывается при расчете `SORT` у остальных привязок ||
|#

### Параметр items {#company_contact_binding}
...
Метод не проверяет, существует ли контакт: с идентификатором несуществующего контакта привязка все равно создается ||
|| **IS_PRIMARY**
[`char`](../../../data-types.md#standart-types) | Является ли привязка первичной. Возможные значения:
- `Y` — да
- `N` — нет

Флаг относится к контакту и означает, что эта компания для него основная.

Новая привязка всегда получает `IS_PRIMARY = Y` независимо от переданного значения. У прежней основной компании этого контакта флаг сбрасывается в `N`.

У привязок, которые уже были у компании, флаг выставляется по переданному набору: `Y` сохраняется за первой привязкой с `IS_PRIMARY = Y`, а если значение `Y` не передали ни в одной привязке — за первой привязкой в `items`. У остальных ранее существовавших привязок флаг сбрасывается в `N`.

Поэтому, когда в `items` есть и новые, и уже существующие привязки, `IS_PRIMARY = Y` может оказаться сразу у нескольких контактов
||
|| **SORT**
[`integer`](../../../data-types.md) | Индекс сортировки.

Если `SORT` не передан, метод вычисляет его по позиции привязки в `items` как `(n + 1) * 10`, где `n` — порядковый номер привязки в `items`, отсчет начинается с нуля. Первая привязка получает `10`, вторая — `20` и так далее.

Это правило действует и для уже существующих привязок: их прежний индекс сортировки перезаписывается. Чтобы сохранить индекс, передайте `SORT` явно ||
|#
