...
>
> Кто может выполнять метод: пользователь с правом чтения компаний
...
- Go
    ...
    res, err := client.Core().Call(ctx, "crm.company.userfield.get", b24.Params{
    	"id": 399,
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("crm.company.userfield.get: %w", err)
    }
...
`тип` | **Описание** ||
|| **result**
[`object`](../../../data-types.md) | Корневой элемент ответа, содержит информацию о полях пользовательского поля. Итоговый перечень полей зависит от типа поля, подробное описание полей можно найти в методе [crm.company.userfield.add](./crm-company-userfield-add.md)||
|| **time**
...
- у пользователя нет прав на чтение компаний
- пользователь пытается получить пользовательское поле, не привязанное к компаниям ||
