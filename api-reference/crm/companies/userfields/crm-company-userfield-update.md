# Обновить пользовательское поле компаний crm.company.userfield.update
...
Идентификатор можно получить с помощью методов [crm.company.userfield.add](./crm-company-userfield-add.md) и [crm.company.userfield.list](./crm-company-userfield-list.md) ||
|| **fields***
...
- crm
    ...
    - `N` — нет ||
    || **COMPANY**
    [`boolean`](../../../data-types.md) | Включена ли привязка к [Компаниям](../index.md). Возможные значения:
    - `Y` — да
    - `N` — нет ||
    || **DEAL**
...
- пользователь пытается изменить пользовательское поле, не привязанное к компаниям ||
