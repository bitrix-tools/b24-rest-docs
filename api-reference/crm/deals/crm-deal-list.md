...
`тип` | **Описание** ||
|| **result**
[`array`](../../data-types.md) | Корневой элемент ответа. Содержит массив содержащий информацию о полях сделок
[Структура отдельно взятой сделки](crm-deal-get.md#result) аналогична результату методу `crm.deal.get`, за исключением того, что у данного метода есть параметр `select` конфигурирующий возвращаемые поля ||
|| **total**
