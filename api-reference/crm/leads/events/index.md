...
Все три события лида передают в объекте `data.FIELDS` только идентификатор лида:
...
Значений полей и списка изменений в событии нет. Чтобы узнать, что именно изменилось, запросите лид методом [crm.lead.get](../crm-lead-get.md) и сравните данные со своей копией. После события `onCrmLeadDelete` этот метод вернет ошибку `Not found`: лида в Битрикс24 уже нет.
