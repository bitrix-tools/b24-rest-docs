...
- Python
    ...
    try:
        bitrix_response = client.userfieldconfig.delete(
            module_id="crm",
            bitrix_id=6953,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "userfieldconfig.delete", b24.Params{
    	"moduleId": "crm",
    	"id":       6953,
    })
    if err != nil {
    	return fmt.Errorf("userfieldconfig.delete: %w", err)
    }
...
|| Пустое значение | Вы не можете удалить пользовательское поле | Недостаточно прав на удаление поля. Эта же ошибка возвращается, если поле с переданным `id` уже удалено или недоступно в контексте `moduleId` ||
...
|| Пустое значение | Ошибка при попытке изменения настроек пользовательских полей | Общая ошибка удаления поля ||
|#
