# Сохранить товарные позиции объекта CRM crm.item.productrow.set
...
>
> Кто может выполнять метод: требуется право на изменение объекта CRM, товарные позиции которого сохраняются

Метод `crm.item.productrow.set` сохраняет набор товарных позиций объекта CRM.

{% note warning "" %}

Метод заменяет все товарные позиции объекта CRM на переданный набор. Позиции, которых нет в `productRows`, удаляются вместе со своими строками в [оплатах](../payment/products-in-payment/index.md), а сумма объекта CRM пересчитывается. Если передать пустой массив, объект CRM останется без товарных позиций.

Чтобы добавить одну позицию, не изменяя остальные, используйте метод [crm.item.productrow.add](./crm-item-productrow-add.md).

{% endnote %}

Набор сохраняется целиком: если хотя бы одна позиция не прошла проверку, метод вернет ошибку `INVALID_ARG_VALUE` и не изменит товарные позиции объекта CRM.
...
|| **productRows***
[`object[]`](../../../data-types.md) | Массив объектов с информацией о товарных позициях, которые нужно сохранить в объекте [(подробное описание)](#productRows) ||
|#

### Параметр productRows {#productRows}

Передайте в каждой позиции `productId` (идентификатор товара из каталога) или `productName` (название произвольной позиции).

#|
|| **Название**
`тип` | **Описание** ||
|| **id**
[`crm_item_product_row.id`](../../data-types.md#crm_item_product_row) | Идентификатор существующей товарной позиции. Получить его можно методом [crm.item.productrow.list](./crm-item-productrow-list.md).
Передайте `id`, чтобы сохранить позицию с тем же идентификатором. Без `id` позиция создается заново.
Если передать `id` несуществующей позиции или позиции другого объекта CRM, метод создаст новую позицию и не вернет ошибку ||
|| **productId**
...
Поля `priceAccount`, `priceExclusive`, `priceNetto`, `priceBrutto`, `measureName`, `type`, `customized`, `xmlId` и `storeId` метод рассчитывает сам. Переданные значения этих полей метод игнорирует без ошибки: обратная отправка позиции, полученной методом [crm.item.productrow.list](./crm-item-productrow-list.md), их не изменит.
...
`тип` | **Описание** ||
|| **productRows**
[`crm_item_product_row[]`](../../data-types.md#crm_item_product_row) | Массив объектов с информацией о всех товарных позициях объекта CRM после сохранения. Метод возвращает их сразу, без постраничной навигации ||
|#
...
|| `ACCESS_DENIED` | У пользователя нет права на изменение объекта CRM ||
