...
Методы `crm.item.payment.*` работают с оплатами, привязанными к универсальным объектам CRM. Например, можно создать оплату для сделки, отметить поступление средств или сформировать ссылку для оплаты клиентом.
...
Оплаты поддерживают не все объекты CRM.
...
Для остальных объектов, например для лида или смарт-процесса, метод [crm.item.payment.add](./crm-item-payment-add.md) вернет ошибку «Для данной сущности функционал создания оплат недоступен», а [crm.item.payment.list](./crm-item-payment-list.md) — пустой список.
...
**Товарные позиции оплаты.** Каждая оплата может содержать набор товарных позиций. Составом оплаты управляет группа методов [crm.item.payment.product.*](./products-in-payment/index.md).

**Позиции доставки.** Оплата может быть связана с позициями доставки. С ними работает группа методов [crm.item.payment.delivery.*](./delivery-in-payment/index.md).
...
3. При необходимости добавьте в оплату товары группой методов [crm.item.payment.product.*](./products-in-payment/index.md) или услуги доставки группой методов [crm.item.payment.delivery.*](./delivery-in-payment/index.md).
4. Получите список оплат объекта методом [crm.item.payment.list](./crm-item-payment-list.md) и выберите нужную оплату по `id`.
5. Получите детали конкретной оплаты методом [crm.item.payment.get](./crm-item-payment-get.md) и при необходимости обновите данные методом [crm.item.payment.update](./crm-item-payment-update.md).
6. Измените статус оплаты методами [crm.item.payment.pay](./crm-item-payment-pay.md) или [crm.item.payment.unpay](./crm-item-payment-unpay.md).
7. Если нужна ссылка для оплаты клиентом, используйте [salescenter.payment.getPublicUrl](./salescenter-payment-get-public-url.md).
8. Если оплата не нужна, удалите ее методом [crm.item.payment.delete](./crm-item-payment-delete.md).
