...
`тип` | **Описание** ||
|| **entityTypeId***
[`integer`][1] | Идентификатор [системного](../data-types.md#object_type) или [пользовательского типа](./user-defined-object-types/index.md), чей элемент мы хотим изменить.
...
- Сделка
  ...
  Список доступных типов объекта можно узнать с помощью [`crm.status.list`][2] применив фильтр `{ ENTITY_ID: "DEAL_TYPE" }` ||
  || **categoryId**
...
- Контакт
  ...
  Список доступных типов объекта можно узнать с помощью [`crm.status.list`][2] применив фильтр `{ ENTITY_ID: "CONTACT_TYPE" }`  ||
  || **sourceId**
  [`crm_status`](../data-types.md) | Строковый идентификатор источника.
...
- Компания
  ...
  Список доступных типов объекта можно узнать с помощью [`crm.status.list`][2] применив фильтр `{ ENTITY_ID: "COMPANY_TYPE" }` ||
  || **logo**
  [`file`][1] | Логотип ||
  || **bankingDetails**
