...
`тип` | **Описание** ||
|| **entityTypeId***
[`integer`](../../data-types.md) | Идентификатор [системного](../data-types.md#object_type) или [пользовательского типа](./user-defined-object-types/index.md), чей элемент мы хотим изменить.
...
|| **id^*^**
[`integer`](../../data-types.md) | Идентификатор элемента, который мы хотим изменить.
...
У каждого типа объекта CRM свой набор полей. Это значит, что набор полей для изменения Лида не обязан подходить набору полей для изменения Контакта или Смарт-процесса.

Список доступных полей относительно каждого типа объекта описан [ниже](#parametr-fields).
...
- Сделка
  ...
  [`string`](../../data-types.md) | Название элемента ||
  || **typeId**
  [`crm_status`](../data-types.md) | Строковый идентификатор типа объекта.
...
- Контакт
  ...
  - `valueType` — подтип значения: `WORK`, `MOBILE`, `HOME`, `MAILING`, `OTHER`
  - `value` — значение

  Ключ элемента в объекте определяет операцию:

  **Добавить** новое значение — использовать ключи `n0`, `n1`, `n2` ...:

  ```json
  "fm": {
      "n0": { "typeId": "PHONE", "valueType": "WORK", "value": "+79991234567" },
      "n1": { "typeId": "EMAIL", "valueType": "WORK", "value": "new@example.com" }
  }
  ```

  **Обновить** существующее значение — использовать числовой `id` записи (берется из ответа [`crm.item.get`](./crm-item-get.md) в поле `fm`):

  ```json
  "fm": {
      "15": { "typeId": "PHONE", "valueType": "MOBILE", "value": "+79990000000" }
  }
  ```

  **Удалить** значение — передать числовой `id` записи с пустым `value`:
  ...
  Операции можно комбинировать в одном запросе.
...
1. Загрузить новый файл вместо старого (не множественное поле)
    ...
    Чтобы заменить файл в одиночном поле, загрузите новый файл. Старый будет удален автоматически.
