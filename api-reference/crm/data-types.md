...
||
|| `crm_dynamic_type.id` | Целочисленный идентификатор смарт-процесса. Получить идентификаторы можно с помощью метода [crm.type.list](./universal/user-defined-object-types/crm-type-list.md) ||
|| `crm_userfield` | Значение пользовательского поля. Конкретный тип значения зависит от типа самого поля — смотрите [{#T}](./universal/user-defined-fields/crm-userfield-types.md) ||
|| `userFieldEntityId` | Строковый код типа объекта CRM, к которому привязано пользовательское поле. Например, `CRM_CONTACT`. Полный список кодов — в разделе [Тип объекта CRM](#object_type) ||
|| `location` | Целочисленный идентификатор местоположения из справочника местоположений Битрикс24. Служебный тип: поля с ним не рекомендуются к использованию, в большинстве объектов они помечены как устаревшие ||
...
|| `recurring_params` | Объект с параметрами расчета даты следующего повторения для шаблона регулярного объекта CRM. Состав полей зависит от объекта и описан на странице конкретного метода. Например, [поля объекта PARAMS](./deals/recurring-deals/crm-deal-recurring-fields.md#params-fields) для регулярных сделок ||
...
|| [`diskfile`](#diskfile) | Файл, сохраненный на Диске Битрикс24. В ответе метод возвращает объект с описанием файла, при записи значение передается в виде структуры загрузки ||
|| [`attached_diskfile`](#attached_diskfile) | Файл, прикрепленный к записи таймлайна. В ответе метод возвращает объект с описанием файла, при записи значение передается в виде структуры загрузки ||
|| `product_file` | Картинка товара в старой карточке товара. В ответе метод возвращает объект с описанием файла ||
|| [`crm_activity_binding`](#crm_activity_binding) | Объект, описывающий привязку дела к элементу CRM ||
|| `crm_activity_communication` | Объект, описывающий канал коммуникации дела. Состав полей — в статье [{#T}](./timeline/activities/activity-base/crm-activity-communication-fields.md) ||
|#

## Перечисления дел {#activity-enums}

Поля дел ссылаются на служебные перечисления. Значение поля — целое число из фиксированного набора.

#|
|| **Тип** | **Значения** ||
|| `crm_enum_activitytype` | Тип дела: `1` — встреча, `2` — звонок, `3` — задача, `4` — письмо, `5` — общий тип для импорта событий календаря, `6` — общий тип для дел, которыми управляют провайдеры. `0` — тип не определен ||
|| `crm_enum_activitystatus` | Статус дела: `1` — ожидает выполнения, `2` — выполнено, `3` — выполнено автоматически. `0` — статус не определен ||
|| `crm_enum_activitypriority` | Важность дела: `1` — низкая, `2` — средняя, `3` — высокая. `0` — не задана ||
|| `crm_enum_activitynotifytype` | Единица измерения времени для напоминания: `1` — минуты, `2` — часы, `3` — дни. `0` — напоминание не задано ||
|| `crm_enum_activitydirection` | Направление дела: `1` — входящее, `2` — исходящее. `0` — направление не определено. Актуально для звонков и писем ||
|| `crm_enum_contenttype` | Формат текста описания: `1` — простой текст, `2` — BB-коды, `3` — HTML. `0` — формат не определен ||
|| `crm_enum_ownertype` | Целочисленный идентификатор типа объекта CRM, которому принадлежит запись. Возможные значения возвращает метод [crm.enum.ownertype](./auxiliary/enum/crm-enum-owner-type.md) ||
|#
...
|| **taxRate**
[`double`](../data-types.md) | Ставка налога в процентах. Возвращается `null`, если налог для позиции не задан ||
|| **taxName**
[`string`](../data-types.md) | Название налоговой ставки. Например, `НДС 20` или `Без НДС` ||
|| **taxIncluded**
...
|| **measureName**
[`string`](../data-types.md) | Текстовое представление единицы измерения. Например, шт, кг, м, л ||
|| **sort**
...
- `5` — торговое предложение, у которого нет товара: товар не указан или удален
- `6` — специфический тип, означает невалидный товар с торговыми предложениями
- `7` — услуга ||
|| **storeId**
[`integer`](../data-types.md) | Идентификатор склада. Заполняется, только если по товарной позиции есть резерв, иначе возвращается `null`.
Поле возвращают методы [crm.item.productrow.get](./universal/product-rows/crm-item-productrow-get.md) и [crm.item.productrow.list](./universal/product-rows/crm-item-productrow-list.md) ||
|#
...
Файл, сохраненный на Диске Битрикс24. В ответе метод возвращает объект с описанием файла.
...
|| **FILE_ID**
[`integer`](../data-types.md) | Идентификатор файла в файловом хранилище ||
|| **NAME**
[`string`](../data-types.md) | Имя файла ||
|| **SIZE**
[`string`](../data-types.md) | Размер файла в читаемом виде. Например, `43 Кб` ||
|| **BYTES**
[`integer`](../data-types.md) | Размер файла в байтах ||
|| **CAN_READ**
[`boolean`](../data-types.md) | Доступен ли файл текущему пользователю на чтение ||
|| **VIEW_URL**
[`string`](../data-types.md) | Адрес для просмотра или скачивания файла ||
|| **PREVIEW_URL**
[`string`](../data-types.md) | Адрес превью. Заполняется только для картинок и видео, в остальных случаях — пустая строка ||
|#

Чтобы записать значение в поле такого типа, передайте структуру загрузки файла по [правилам работы с файлами](../files/how-to-upload-files.md).

### attached_diskfile {#attached_diskfile}

Файл, прикрепленный к записи таймлайна. От [`diskfile`](#diskfile) отличается тем, что файл не лежит в объекте CRM, а привязан к записи как вложение.

В ответе метод возвращает объект с описанием файла: идентификатор вложения, дату, тип, имя, размер и — для картинок — размеры изображения.

Чтобы записать значение, передайте структуру загрузки файла по [правилам работы с файлами](../files/how-to-upload-files.md).

### crm_activity_binding {#crm_activity_binding}

Привязка дела к элементу CRM. Поле множественное и доступно только для чтения.

#|
|| **Название**
`тип` | **Описание** ||
|| **OWNER_TYPE_ID**
[`crm_enum_ownertype`](#activity-enums) | Тип объекта CRM, к которому привязано дело ||
|| **OWNER_ID**
...
### Формат значений для пользовательского поля «Привязка к элементам CRM» {#crm-binding-format}

Пользовательские поля типа `crm` принимают значения в формате `{PREFIX}_{ID}`, где `PREFIX` — краткий символьный код типа (`entityTypeAbbr`) из таблицы выше, `ID` — числовой идентификатор элемента.

**Одиночная привязка** — строка:

#|
|| **Тип объекта** | **Пример значения** ||
|| Лид | `L_4` ||
|| Сделка | `D_1` ||
|| Контакт | `C_2` ||
|| Компания | `CO_3` ||
...
**Множественная привязка** (поле с `isMultiple: true`) — массив строк:

```json
["C_123", "CO_456"]
```

Для смарт-процессов PREFIX вычисляется из `entityTypeId`: значение переводится в шестнадцатеричное представление в нижнем регистре, затем добавляется префикс `T`. Пример: `entityTypeId = 128` → `80` → PREFIX `T80`.
