...
Метод `crm.quote.userfield.add` создает новое пользовательское поле для предложений.
...
||
|| **FIELD_NAME***
[`string`](../../../data-types.md) | Код поля. Уникальный в пределах коммерческих предложений.
...
- Go
    ...
    res, err := client.Core().Call(ctx, "crm.quote.userfield.add", b24.Params{
    	"fields": b24.Params{
    		"LABEL":        "Поле Привет, мир!",
    		"USER_TYPE_ID": "string",
    		"FIELD_NAME":   "HELLO_WORLD",
    ...
    if err != nil {
    	return fmt.Errorf("crm.quote.userfield.add: %w", err)
    }
