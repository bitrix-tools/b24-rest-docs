...
- Go
    ...
    res, err := client.Core().Call(ctx, "crm.quote.delete", b24.Params{
    	"id": 43,
    })
    if err != nil {
    	return fmt.Errorf("crm.quote.delete: %w", err)
    }
...
|| Пустое значение | `Access denied.` | У пользователя нет прав на удаление коммерческих предложений ||
