...
>
> Пользовательская документация: [Шаблоны документов в CRM: как быстро отправлять счета и акты](https://helpdesk.bitrix24.ru/open/18089278/)
...
- При загрузке шаблона обязательны `name`, `file`, `numeratorId`, `region` и `entityTypeId`. Если пропустить любой из них, метод [crm.documentgenerator.template.add](./crm-document-generator-template-add.md) вернет ошибку `Empty required fields`
- Доступ к шаблону задает параметр `users` с кодами доступа, например `["UA"]` — все пользователи. Если параметр не передать, в доступ попадет только текущий пользователь
- Привязка шаблона к воронке при создании документа не проверяется: документ создастся, даже если шаблон настроен для другой воронки
- Методы `crm.documentgenerator.template.*` работают с шаблонами CRM. Шаблонами вне CRM управляют методы раздела [Генератор документов](../../../document-generator/templates/index.md)
...
**CRM-объекты.** Шаблон привязывают к одному или нескольким типам объектов через `entityTypeId`. Значения для стандартных объектов приведены в таблице [Тип объекта CRM](../../data-types.md#object_type), а для смарт-процессов `entityTypeId` можно получить методом [crm.type.list](../../universal/user-defined-object-types/crm-type-list.md). Чтобы уточнить воронку, к идентификатору добавляют суффикс, например `2_category_0`.
