...
Метод `crm.documentgenerator.numerator.add` создает новый нумератор.
...
- название — `Нумератор из REST`
- шаблон — `{NUMBER}`
- стартовая нумерация — с `1`, шаг `1`
...
- cURL (Webhook)
    ...
    -d '{"fields":{"name":"Нумератор из REST","template":"{NUMBER}","settings":{"Bitrix_Main_Numerator_Generator_SequentNumberGenerator":{"start":1,"step":1,"length":6,"padString":"0","periodicBy":"","timezone":"","isDirectNumeration":false}}}}' \
...
- cURL (OAuth)
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"fields":{"name":"Нумератор из REST","template":"{NUMBER}","settings":{"Bitrix_Main_Numerator_Generator_SequentNumberGenerator":{"start":1,"step":1,"length":6,"padString":"0","periodicBy":"","timezone":"","isDirectNumeration":false}}},"auth":"**put_access_token_here**"}' \
...
|| `0` | `Empty required fields: ...` | Не переданы обязательные поля ||
