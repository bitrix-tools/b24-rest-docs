...
## Структура конфигурации карточки

Конфигурация — это массив секций. Каждая секция содержит массив полей контакта. Например, конфигурация одной секции с именем, фамилией и телефоном выглядит так:
...
|| Секция | `name`, `title`, `type`, `elements` | `name` — уникальное имя секции, `title` — заголовок в карточке, `type` принимает значение `section`, `elements` содержит поля контакта ||
|| Поле секции | `name`, `optionFlags`, `options` | `name` — идентификатор поля контакта, `optionFlags` определяет постоянное отображение поля, `options` содержит дополнительные настройки поля ||
|#

Метод [crm.contact.details.configuration.set](./crm-contact-details-configuration-set.md) принимает эту структуру в параметре `data`. Метод [crm.contact.details.configuration.get](./crm-contact-details-configuration-get.md) возвращает массив с такой же структурой в поле `result`.
