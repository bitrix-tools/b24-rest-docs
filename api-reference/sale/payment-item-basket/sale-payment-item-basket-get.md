...
Метод `sale.paymentitembasket.get` получает значения всех полей привязки элемента корзины к оплате.
...
- Python
    ...
    try:
        bitrix_response = client.sale.paymentitembasket.get(
            bitrix_id=1186,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "sale.paymentitembasket.get", b24.Params{
    	"id": 1186,
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("sale.paymentitembasket.get: %w", err)
    }
