...
Метод `sale.paymentitembasket.list` получает список привязок элементов корзины к оплатам.
...
- Python
    ...
    try:
        bitrix_response = client.sale.paymentitembasket.list(
            select=[
                "id",
                "paymentId",
                "basketId",
...
- Go
    ...
    // Метод заворачивает ответ в объект с ключом "paymentItemsBasket".
    raw, ok := b24.Unwrap(res.Result, "paymentItemsBasket")
    if !ok {
    	return fmt.Errorf("в ответе нет ключа paymentItemsBasket")
    }
