...
Метод `sale.property.get` получает свойство заказа.
...
- Python
    ...
    try:
        bitrix_response = client.sale.property.get(
            bitrix_id=22,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "sale.property.get", b24.Params{
    	"id": 22,
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("sale.property.get: %w", err)
    }
