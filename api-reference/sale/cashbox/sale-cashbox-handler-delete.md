...
Метод `sale.cashbox.handler.delete` удаляет REST-обработчик кассы.

Если к обработчику привязаны кассы, то обработчик не будет удален — сначала необходимо удалить кассы.
...
`тип` | **Описание** ||
|| **ID***
[`sale_cashbox_handler.ID`](../data-types.md#sale_cashbox_handler) | Идентификатор удаляемого обработчика ||
|#
...
- Python
    ...
    try:
        bitrix_response = client.sale.cashbox.handler.delete(
            bitrix_id=1,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "sale.cashbox.handler.delete", b24.Params{
    	"ID": 1,
    })
    if err != nil {
    	return fmt.Errorf("sale.cashbox.handler.delete: %w", err)
    }
...
`тип` | **Описание** ||
|| **result**
[`boolean`](../../data-types.md) | Результат удаления REST-обработчика кассы ||
|| **time**
...
|| `ERROR_CHECK_FAILURE` | Не указано значение поля `ID` либо на портале остались кассы, привязанные к этому обработчику | 400 ||
|| `ERROR_HANDLER_NOT_FOUND` | Обработчик с указанным `ID` не найден | 400 ||
|| `ERROR_HANDLER_DELETE` | Прочие ошибки. Более подробную информацию об ошибке можно найти в `error_description` | 400 ||
|#
