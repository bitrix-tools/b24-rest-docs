...
Метод `sale.basketproperties.list` возвращает набор свойств элементов корзины, выбранных по фильтру.
...
|| **filter**
[`object`](../../data-types.md) | Объект для фильтрации выбранных свойств корзины в формате `{"field_1": "value_1", ... "field_N": "value_N"}`. Если не передать, метод вернет свойства всех позиций корзины.
...
Если `order` не передать, свойства придут по возрастанию `id`, а не по полю `sort`. Чтобы получить свойства в порядке `sort`, передайте `{"sort": "asc", "id": "asc"}`
||
|| **start**
...
Если не передать, метод вернет первую страницу
||
|#

{% note warning "" %}

Неизвестные поля в `select`, `filter` и `order` метод пропускает без ошибки. Если ошибиться в имени поля фильтра, например написать `basketid` вместо `basketId`, метод вернет свойства всех позиций корзины.
...
- Python
    ...
    try:
        bitrix_response = client.sale.basketproperties.list(
            select=[
                "id",
                "basketId",
                "value",
...
|| **basketProperties**
[`sale_basket_item_property[]`](../data-types.md#sale_basket_item_property) | Массив объектов с информацией о выбранных свойствах элементов (позиций) корзины в заказах. Если свойств не найдено, массив пустой ||
|| **total**
