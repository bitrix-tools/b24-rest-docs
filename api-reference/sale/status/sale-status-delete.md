...
Метод `sale.status.delete` удаляет статус заказа или доставки.
...
- Python
    ...
    try:
        bitrix_response = client.sale.status.delete(
            bitrix_id='MS',
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "sale.status.delete", b24.Params{
    	"id": "MS",
    })
    if err != nil {
    	return fmt.Errorf("sale.status.delete: %w", err)
    }
