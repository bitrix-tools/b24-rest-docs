...
1. Выберите тип статуса: `O` для заказа или `D` для доставки.
2. Проверьте существующие статусы методом [sale.status.list](./sale-status-list.md).
3. Создайте новый статус методом [sale.status.add](./sale-status-add.md), если статусов по умолчанию недостаточно.
4. Добавьте названия статуса для нужных языков методами [sale.statusLang.*](../status-lang/index.md).
