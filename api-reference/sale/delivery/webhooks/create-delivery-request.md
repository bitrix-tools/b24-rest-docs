...
Битрикс24 отправляет запрос на адрес из параметра `CREATE_DELIVERY_REQUEST_URL`, который передан при создании обработчика доставки методом [sale.delivery.handler.add](../handler/sale-delivery-handler-add.md). Внешняя система должна создать заказ на доставку и вернуть его идентификатор.
