...
- Метод [sale.propertyvalue.modify](./sale-property-value-modify.md) принимает набор значений свойств заказа и сохраняет его как актуальный набор значений. Перед обновлением получите текущие значения, чтобы не потерять данные, которые должны остаться в заказе.
