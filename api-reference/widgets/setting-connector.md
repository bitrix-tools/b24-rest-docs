...
Обработчик подключается не методом `placement.bind`, а параметром `PLACEMENT_HANDLER` метода [imconnector.register](../imopenlines/imconnector/imconnector-register.md). Битрикс24 сам создает привязку встройки, когда регистрирует коннектор.
...
- Битрикс24 создаст привязку встройки `SETTING_CONNECTOR` и свяжет ее с коннектором
