...
Команда `CallCardSetMute` включает или отключает микрофон оператора в карточке звонка.
...
Если микрофон уже находится в запрошенном состоянии, команда сразу возвращает пустой массив: карточка не меняется и событие [BackgroundCallCard::muteButtonClick](./events/mute-button-click.md) не возникает.
...
|| `missing field muted` | Не передан обязательный параметр `muted` | В десктоп-сценарии поле `muted` обязательно. Этот код приходит одним объектом, без обертки в массив ||
|#

Если команда вызвана в другой точке встраивания, функция обратного вызова не будет вызвана вовсе: неизвестную команду интерфейс точки встраивания игнорирует. Проверить, что команда `CallCardSetMute` доступна, можно методом [BX24.placement.getInterface](../bx24-placement-get-interface.md).
