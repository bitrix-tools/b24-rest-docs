# При нажатии на кнопку выключения микрофона BackgroundCallCard::muteButtonClick
...
Событие `BackgroundCallCard::muteButtonClick` возникает при нажатии оператором на кнопку управления микрофоном.
...
`тип` | **Описание** ||
|| **eventData***
[`boolean`](../../../../data-types.md) | Текущее состояние микрофона после нажатия кнопки.
...
Для данного события — `BackgroundCallCard::muteButtonClick` ||
|| **callback***
...
- JS (TS)
    ...
    await $b24.placement.bindEvent('BackgroundCallCard::muteButtonClick', (eventData: boolean) => {
      console.log(eventData)
    })
    ```
...
- JS (UMD)
    ...
    <script>
      document.addEventListener('DOMContentLoaded', async () => {
        ...
        await $b24.placement.bindEvent('BackgroundCallCard::muteButtonClick', (eventData) => {
          console.log(eventData)
        })
      })
    </script>
