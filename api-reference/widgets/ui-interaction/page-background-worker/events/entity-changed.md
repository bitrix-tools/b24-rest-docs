# При загрузке или смене объекта CRM BackgroundCallCard::entityChanged
...
Событие `BackgroundCallCard::entityChanged` возникает, когда карточка загружает или меняет связанный со звонком объект CRM.

Событие приходит в трех случаях:

- карточка звонка открылась и подтянула данные клиента из CRM. В этом случае событие может прийти раньше, чем [BackgroundCallCard::initialized](./initialized.md)
- клиента опознали или звонок перепривязали к другому объекту CRM
...
Для данного события — `BackgroundCallCard::entityChanged` ||
|| **callback***
