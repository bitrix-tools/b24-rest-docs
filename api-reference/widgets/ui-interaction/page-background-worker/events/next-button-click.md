...
Событие `BackgroundCallCard::nextButtonClick` возникает при переходе к следующему клиенту в режиме обзвона.
...
Для данного события — `BackgroundCallCard::nextButtonClick` ||
|| **callback***
...
- BX24.js
    ...
    BX24.ready(function () {
        BX24.init(function () {
            BX24.placement.bindEvent('BackgroundCallCard::nextButtonClick', function () {
                // код обработчика
            });
        });
    });
...
- JS (TS)
    ...
    await $b24.placement.bindEvent('BackgroundCallCard::nextButtonClick', () => {
      // код обработчика
    })
    ```
