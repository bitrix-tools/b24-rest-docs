...
Событие `BackgroundCallCard::notifyAdminButtonClick` возникает при запросе уведомления администратора из карточки звонка.
...
Для данного события — `BackgroundCallCard::notifyAdminButtonClick` ||
|| **callback***
...
- BX24.js
    ...
    BX24.ready(function () {
        BX24.init(function () {
            BX24.placement.bindEvent('BackgroundCallCard::notifyAdminButtonClick', function () {
                // код обработчика
            });
        });
    });
...
- JS (TS)
    ...
    await $b24.placement.bindEvent('BackgroundCallCard::notifyAdminButtonClick', () => {
      // код обработчика
    })
    ```
