# При нажатии на кнопку «позвонить» или «перезвонить» BackgroundCallCard::makeCallButtonClick
...
Событие `BackgroundCallCard::makeCallButtonClick` возникает при запуске вызова из карточки звонка.
...
Для данного события — `BackgroundCallCard::makeCallButtonClick` ||
|| **callback***
...
- BX24.js
    ...
    BX24.ready(function () {
        BX24.init(function () {
            BX24.placement.bindEvent('BackgroundCallCard::makeCallButtonClick', function () {
                // код обработчика
            });
        });
    });
...
- JS (TS)
    ...
    await $b24.placement.bindEvent('BackgroundCallCard::makeCallButtonClick', () => {
      // код обработчика
    })
    ```
