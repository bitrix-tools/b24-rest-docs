...
В ссылку можно добавить свои параметры — они придут обработчику в `PLACEMENT_OPTIONS`. Так одно приложение открывает разные экраны: карточку документа, отчет, форму согласования.
...
Ключ `URI` Битрикс24 добавляет сам. Если приложение передаст в `params` свой ключ `URI`, он останется без изменений — подставленное значение его не перезапишет.
...
**Контент, в котором живет ссылка.** Приложение размещает ссылку теми же методами, что и любой другой текст: сообщение чата — методами раздела [{#T}](../../chats/index.md), сообщение ленты — методами раздела [{#T}](../../log/index.md), описание задачи — методами раздела [{#T}](../../tasks/index.md).
