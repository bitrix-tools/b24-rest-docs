...
Название пункта и название группы задаются параметрами метода [placement.bind](../placement-bind.md) при регистрации.

#|
|| **Параметр**
`тип` | **Описание** ||
|| **TITLE**
[`string`](../../data-types.md) | Название пункта в меню. Если параметр не передан, вместо него выводится название приложения ||
|| **GROUP_NAME**
[`string`](../../data-types.md) | Название группы, в которой будет пункт. Если параметр не передан, пункт попадает в группу *Приложения*. Пункты нескольких приложений с одинаковым `GROUP_NAME` собираются в одну группу ||
|#

Оба названия переводятся на язык пользователя через параметр `LANG_ALL` — как в примерах ниже.
...
|| Виджет зарегистрирован, но пункта в меню аналитики нет | Завершите [установку приложения](../../../settings/app-installation/installation-finish.md) и перезагрузите страницу Битрикс24: список пунктов меню собирается при загрузке страницы ||
|#
