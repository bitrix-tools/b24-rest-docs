...
Точка объявлена в глобальном скоупе `placement` — отдельного мобильного скоупа у нее нет. Скоуп `im` нужен обработчику, чтобы работать с чатом по полученному `dialogId`.
...
Кнопка есть не в каждом Битрикс24: ее показ включается на стороне Битрикс24 и не зависит ни от настроек приложения, ни от параметров регистрации. Пока кнопки нет, зарегистрированный виджет не выводится, хотя сама регистрация работает.
...
#|
|| **Ошибка** | **Как решить** ||
|| Кнопки *Приложения* нет в чате | Обновите мобильное приложение: в старых версиях ряда кнопок над полем ввода нет вовсе. Если кнопки нет и в свежей версии, ее показ еще не включен для этого Битрикс24 — вывести виджет в этом случае нельзя ||
|| Меню открывается, но пункта приложения в нем нет | Завершите [установку приложения](../../settings/app-installation/installation-finish.md). Пока установка не завершена, виджет не выводится ||
