# Дополнительные возможности точки CRM_XXX_DETAIL_ACTIVITY, CRM_DYNAMIC_XXX_DETAIL_ACTIVITY
...
>
> Кто может работать с виджетом: пользователь с доступом на изменение элемента

Точки `CRM_XXX_DETAIL_ACTIVITY` и `CRM_DYNAMIC_XXX_DETAIL_ACTIVITY` добавляют пункт приложения над таймлайном карточки элемента CRM. Вместо собственного интерфейса в iframe приложение может показать в этом пункте штатный интерфейс Битрикс24: текст, ссылки, поля ввода, списки и кнопки.

Для регистрации виджета используйте метод [placement.bind](../placement-bind.md). Базовые возможности точки описаны в статье [Кнопка над таймлайном карточки элемента](./detail-activity.md).

[Скачать пример приложения с использованием этой точки.](https://helpdesk.bitrix24.ru/examples/timeline_activity_placement_einvoice.zip)

## Куда встраивается виджет

#|
|| **Код точки встраивания** | **Место** ||
|| `CRM_XXX_DETAIL_ACTIVITY` | Пункт над таймлайном карточки стандартного объекта CRM. Вместо `XXX` укажите тип объекта, например `DEAL` ||
|| `CRM_DYNAMIC_XXX_DETAIL_ACTIVITY` | Пункт над таймлайном карточки пользовательского типа объектов CRM. Вместо `XXX` укажите числовой идентификатор типа объекта ||
|#

Пользователь открывает пункт приложения в карточке элемента. Если при регистрации передано `useBuiltInInterface: Y`, приложение сначала загружается в скрытом iframe, а затем формирует интерфейс вызовом `setLayout`.
...
При загрузке приложения Битрикс24 передает в `placementOptions` контекст текущего элемента:
...
|| **entityId**
[`integer`](../../data-types.md) | Идентификатор элемента CRM, в карточке которого открыт виджет ||
|| **useBuiltInInterface**
[`string`](../../data-types.md) | Признак использования штатного интерфейса. Для описанного на странице сценария имеет значение `Y` ||
|#
...
## Работа с интерфейсом виджета {#Interface}
...
2. Отрисовка интерфейса. 
   ...
   Когда приложение загружено, оно должно вызвать `setLayout`, чтобы отрисовать первичное состояние виджета.
...
6. Завершение работы.
	...
	Когда процесс работы пользователя с виджетом завершен или если пользователь нажал «отмена», необходимо вызвать `finish`. Таймлайн будет переключен на таб по умолчанию.
...
`тип` | **Описание** ||
|| **blocks**
[`ContentBlockDto[]`](#contentblockdto) | Ассоциативный массив объектов, описывающих контентные блоки. Ключи массива — идентификаторы блоков ||
|| **primaryButton**
...
- `primaryButton` — коллбек `BX24.placement.call('bindPrimaryButtonClickCallback', null, callback)`
...
`тип` | **Описание** ||
|| **value***
[`string`](../../data-types.md) \| [`double`](../../data-types.md) | Текст или число ||
|| **multiline**
[`boolean`](../../data-types.md) | Обработка переносов строк. Если `true`, символы `\n` будут заменены на `<br>`. 
По умолчанию `false` ||
|| **bold**
...
Блок выводит в одну строку несколько контент-блоков типа текст, ссылка или выпадающий список. Это позволяет выводить одной строкой текст с разным форматированием, ссылками и списками.
...
`тип` | **Описание** ||
|| **blocks***
[`ContentBlockDto[]`](#contentblockdto) | Ассоциативный массив контент-блоков. Поддерживаются блоки с типами `text`, `link`, `dropdownMenu` ||
|#
...
`тип` | **Описание** ||
|| **blocks***
[`ContentBlockDto[]`](#contentblockdto) | Ассоциативный массив контент-блоков. Поддерживаются блоки с типами `text`, `link`, `lineOfBlocks` ||
|#
...
`тип` | **Описание** ||
|| **blocks***
[`ContentBlockDto[]`](#contentblockdto) | Ассоциативный массив контент-блоков. Поддерживаются все виды блоков, кроме `section` ||
|| **imageSrc**
...
- `loading` — отображается индикатор загрузки
- `disabled` ||
|#

По умолчанию кнопка активна, параметр `state` можно не передавать.

### ActionDto {#actiondto}

Действие определяет реакцию на нажатие ссылки или другого интерактивного элемента. Доступные виды действий:
...
Вызов действия откроет слайдер приложения, зарегистрировавшего виджет. В слайдер будет передан контекст:
...
|| **value**
[`object`](../../data-types.md) \| [`string`](../../data-types.md) | Данные, которые будут переданы в слайдер приложения. Строковое значение будет доступно в параметре `value` ||
|| **sliderParams**
...
`тип` | **Описание** ||
|| **width**
[`integer`](../../data-types.md) | Ширина слайдера, px. Не может быть использовано одновременно с `leftBoundary` ||
|| **leftBoundary**
[`integer`](../../data-types.md) | Слайдер во всю ширину окна браузера с отступом слева, px. Не может быть использовано одновременно с `width` ||
|| **title**
[`string`](../../data-types.md) | Заголовок слайдера приложения ||
|| **labelText**
[`string`](../../data-types.md) | Текст метки в заголовке слайдера ||
|| **labelColor**
[`string`](../../data-types.md) | Цвет текста метки в заголовке слайдера ||
|| **labelBgColor**
[`string`](../../data-types.md) | Цвет фона метки в заголовке слайдера ||
|#
...
|| Пункт приложения открывается как обычный iframe без штатного интерфейса | При регистрации точки передайте `useBuiltInInterface: Y` в объекте `OPTIONS` ||
|| Пункт открыт, но содержимое не появляется | После загрузки приложения вызовите `BX24.placement.call('setLayout', LayoutDto, callback)` и передайте непустой объект `blocks` ||
|| Нажатие на ссылку с действием `layoutEvent` не обрабатывается | Зарегистрируйте коллбек через `bindLayoutEventCallback`. В действии передавайте идентификатор события в поле `value` ||
|| После завершения сценария пункт остается открытым | Вызовите `BX24.placement.call('finish')`, когда пользователь завершил работу или отменил действие ||
|#
