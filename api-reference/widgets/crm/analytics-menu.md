...
|| `placement.bind` возвращает `ERROR_PLACEMENT_NOT_FOUND` | Код указан неверно или приложению не выдан скоуп `crm`. Точка регистрируется только по коду `CRM_ANALYTICS_MENU` ||
...
|| Пункт не виден в левом меню CRM-аналитики | Раскройте пункт *Приложения*: виджет выводится в нем рядом с пунктом *Маркетплейс* ||
|#
