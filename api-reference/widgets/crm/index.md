...
Точки делятся на две группы. Первая работает с конкретным типом объекта, и код точки содержит имя этого типа: `CRM_DEAL_DETAIL_TAB`, `CRM_LEAD_LIST_MENU`. Вторая относится к разделу CRM целиком, и имени объекта в коде нет: `CRM_ANALYTICS_MENU`, `CRM_FUNNELS_TOOLBAR`.
...
1. Выберите точку встраивания под сценарий и соберите ее код: подставьте имя типа объекта, если код его содержит. Для пользовательских типов объектов вместо имени подставляется числовой идентификатор типа — `CRM_DYNAMIC_183_DETAIL_TAB`.
...
|| Код собран для типа объекта, который эту точку не поддерживает | Сверьте код с таблицей раздела: не все типы объектов поддержаны во всех точках ||
