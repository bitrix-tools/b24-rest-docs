...
|| `placement.bind` возвращает `ERROR_PLACEMENT_NOT_FOUND` | Код указан неверно или приложению не выдан скоуп `placement`. Точка регистрируется только по коду `BI_ANALYTICS_MENU` ||
...
|| Пункт видят не все сотрудники | Пункт выводится только тем, кому разрешено работать с BI-аналитикой ||
|#
