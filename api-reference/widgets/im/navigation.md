...
|| Пункт не появился в меню навигации мессенджера | Завершите установку приложения и заново откройте мессенджер ||
|| Пункт не находят в ряду навигации | Пункт приложения добавляется последним, поэтому чаще всего он уезжает под кнопку *Еще*. Откройте этот список ||
|| Показ пункта пытаются ограничить ключом `context` | Точка не привязана к чату, и ключ `context` для нее не применяется. Ограничить доступ можно ключами `role` и `extranet` ||
|#
