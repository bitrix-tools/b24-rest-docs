...
Эта страница — общий список мест. Порядок регистрации, права, состав данных обработчика и типовые ошибки описаны в статье [Механизм встройки виджетов](./index.md).

> Быстрый переход: [все точки встраивания](#all-placements)

## Как выбрать точку встраивания

Одинаковые по смыслу места есть в разных инструментах Битрикс24, поэтому выбирайте точку по задаче, а не по разделу:
...
|| Пункт в главном меню, открывающий основную страницу приложения | Настройки приложения, без вызовов REST API. Где их указать, описано в статье [Механизм встройки виджетов](./index.md). Точка [LEFT_MENU](./left-menu.md) нужна, чтобы пункт открывал зарегистрированный обработчик, а не основной адрес приложения ||
...
|| [IM_SMILES_SELECTOR](./im/smile-selector.md) | Архивная встройка коллекции смайлов. Не используйте для новых интеграций | не требуется ||
...
|| [TASK_VIEW_SIDEBAR](./task/view-sidebar.md) | Встройка в карточке задачи, в прежней карточке — правая панель | `task` ||
|| [TASK_VIEW_TAB](./task/view-tab.md) | Встройка в карточке задачи, в прежней карточке — отдельная вкладка | `task` ||
|| [TASK_VIEW_TOP_PANEL](./task/view-top-panel.md) | Встройка в карточке задачи, в прежней карточке — кнопка верхней панели | `task` ||
