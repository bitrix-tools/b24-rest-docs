# Виджет в карточке задачи TASK_VIEW_TOP_PANEL
...
Виджет добавляет интерфейс приложения в карточку задачи. До версии модуля `tasks` 25.700.0 он выводился кнопкой в верхней панели, в [новой карточке](../../tasks/tasks-new.md) — строкой в блоке *Приложения*.
...
Ограничить вывод виджета задачами конкретных проектов эта точка не позволяет — см. [OPTIONS при регистрации](#options).
...
|| `TASK_VIEW_TOP_PANEL` | Строка в блоке *Приложения* карточки задачи ||
|#
...
![Строка в блоке «Приложения» карточки задачи](./_images/TASK_VIEW_TOP_PANEL.png "Строка в блоке «Приложения» карточки задачи")
...
Эта точка не поддерживает параметр подключения `groupId`: его поддерживают только [TASK_VIEW_TAB](./view-tab.md#options) и [TASK_VIEW_SIDEBAR](./view-sidebar.md#options). Переданные значения не сохраняются: метод [placement.get](../placement-get.md) возвращает для такой регистрации пустой массив.
...
|| Виджет выводится во всех задачах, хотя при регистрации передан `groupId` | Эта точка `groupId` не поддерживает — см. [OPTIONS при регистрации](#options) ||
|#
