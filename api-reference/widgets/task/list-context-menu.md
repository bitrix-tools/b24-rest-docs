...
Виджет добавляет свой пункт в контекстное меню отдельной задачи в списке. Обработчик получает идентификатор задачи.

Точку выбирают, когда над одной задачей нужно выполнить действие без перехода в ее карточку: согласовать, выгрузить, отправить во внешнюю систему.
...
Откройте список задач, нажмите кнопку меню слева от задачи и наведите курсор на *Маркетплейс*. Приложение выводится в этом подменю, а не прямо в контекстном меню задачи.
...
Параметры подключения эта точка не поддерживает: ограничить вывод пункта при регистрации нельзя.

## Примеры кода

{% include [Сноска о примерах](../../../_includes/examples.md) %}

Название пункта в меню задает параметр `TITLE`, его переводы — `LANG_ALL`.
...
|| Обработчик не находит идентификатор задачи | Читайте идентификатор из ключа `ID`. У точек карточки задачи идентификатор приходит в ключе `taskId` ||
|#
