...
При удалении или обновлении приложения Битрикс24 удаляет из списка доступных роботов только роботов, зарегистрированных этим приложением. Роботы других приложений и стандартные роботы не затрагиваются. Если робот уже используется в схеме автоматизации, он остается в схеме как недоступный: его можно удалить, но нельзя настроить или выполнить как доступного робота. Под обновлением здесь понимается обновление приложения, а не вызов метода [bizproc.robot.update](./bizproc-robot-update.md). При повторной установке приложения робот с тем же кодом снова становится доступным.

{% note warning "" %}

На карточке недоступного робота Битрикс24 показывает сообщение «Приложение, из которого был установлен робот, удалено. Робот недоступен». Сообщение появляется всегда, когда робот отсутствует среди доступных, — в том числе после вызова `bizproc.robot.delete` при установленном приложении. Учитывайте это, если удаляете робота из кода: пользователь увидит сообщение об удалении приложения, хотя приложение продолжает работать.
