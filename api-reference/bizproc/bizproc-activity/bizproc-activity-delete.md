...
При удалении или обновлении приложения Битрикс24 удаляет из списка доступных в дизайнере бизнес-процессов только действия, зарегистрированные этим приложением. Действия других приложений и стандартные действия не затрагиваются. Если действие уже используется в схеме бизнес-процесса, оно остается в схеме как недоступное: его можно удалить, но нельзя настроить или выполнить как доступное действие. Под обновлением здесь понимается обновление приложения, а не вызов метода [bizproc.activity.update](./bizproc-activity-update.md). При повторной установке приложения действие с тем же кодом снова становится доступным.
...
- Python
    ...
    try:
        bitrix_response = client.bizproc.activity.delete(
            code="md5_action",
        ).response
