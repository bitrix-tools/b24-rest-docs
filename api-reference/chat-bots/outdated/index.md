...
Если вы разрабатываете интеграции для Битрикс24 с помощью AI-инструментов (ChatGPT, Claude, Copilot), подключите [MCP-сервер](../../../mcp.md), чтобы ассистент использовал официальную REST-документацию.
...
- Методы
    ...
    #|
    || **Метод** | **Описание** ||
    || im.revision.get | Возвращает информацию о ревизиях API ||
...
- Методы
    ...
    || [imbot.chat.setManager](./chats/imbot-chat-set-manager.md) | Назначает или снимает права администратора у участника чата ||
