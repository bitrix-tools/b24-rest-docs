...
Развитие события остановлено. Используйте [ONIMBOTV2MESSAGEDELETE](../../../chat-bots-v2/imbot.v2/events/events.md#onimbotv2messagedelete).
...
Для ботов с `TYPE=B/H` и `OPENLINE=N` параметр `EVENT_MESSAGE_DELETE` игнорируется в методе [imbot.register](../../bots/imbot-register.md). Чтобы событие срабатывало для таких ботов, привяжите обработчик `EVENT_MESSAGE_DELETE` с помощью метода [imbot.update](../../bots/imbot-update.md). 
