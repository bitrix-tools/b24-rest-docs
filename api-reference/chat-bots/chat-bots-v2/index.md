...
Если интеграция уже работает в проде или использует новые поля и методы, сначала проверьте [Журнал изменений API imbot.v2](./change-log.md). Записи в нем расположены от новых к старым.
...
## Навигация по разделу {#navigation}

#|
|| **Если вам нужно** | **Открывайте подраздел** ||
|| Зарегистрировать бота и управлять его свойствами | [Боты](./imbot.v2/bots/index.md) ||
|| Создать групповой чат, управлять участниками и менеджерами | [Чаты](./imbot.v2/chats/index.md) ||
|| Отправлять, изменять и удалять сообщения, работать с реакциями | [Сообщения](./imbot.v2/messages/index.md) ||
|| Зарегистрировать слэш-команды и отвечать на их вызовы | [Команды](./imbot.v2/commands/index.md) ||
|| Показать индикатор действия бота или управлять полем ввода | [Интерфейс чата](./imbot.v2/ui/index.md) ||
|| Получать события бота — сообщения, команды, добавление в чат | [События imbot.v2](./imbot.v2/events/index.md) ||
|| Загружать файлы в чат от имени бота | [Файлы imbot.v2](./imbot.v2/files/index.md) ||
|| Читать события мессенджера от имени пользователя, без регистрации бота | [Работа с чатом im.v2](./im.v2/index.md) ||
|| Понять структуру объектов Bot, Chat, Message, User и File | [Объекты и поля](./entities.md) ||
|| Перенести существующего бота с `imbot.*` на `imbot.v2` | [Миграция с imbot на imbot.v2](./migration.md) ||
|#
...
Тип `personal` рекомендуется для AI-ассистентов, которым нужен полный контекст группового диалога. Боты с этим типом в будущем будут скрыты из поиска для пользователей, у которых к нему нет доступа.
...
`dialogId` используется в методах работы с чатами, сообщениями и файлами `Chat.Message.send`, `Chat.get`, `File.upload` и так далее. Другие методы используют собственные идентификаторы: `botId` (Bot.update, Event.get), `commandId` (Command.update), `fileId` (File.download). Методы `im.v2.Event.*` не используют `dialogId`.
...
Рекомендуемый интервал: **5–30 секунд** при отсутствии новых событий. Если `hasMore = true` — следующий запрос с минимальной паузой в две секунды.

### Webhook-обработчик (webhook-режим) {#webhook-handler}
...
Механизм Bot Context позволяет передавать боту произвольные JSON-данные при открытии чата — через веб-ссылку с параметром `BOT_CONTEXT`, через `bx`-диплинк для приложения или через JS API `Messenger.openChatWithBotContext()`. Бот получает контекст через событие [ONIMBOTV2CONTEXTGET](./imbot.v2/events/events.md#onimbotv2contextget) и может сразу запустить нужный сценарий: открыть карточку, выдать купон или начать авторизацию.

Подробное описание форматов ссылок, событий и рекомендации по безопасности: [Контекст бота](./imbot.v2/bot-context.md).

## Актуальность методов и совместимость {#lifecycle}

Все методы этого раздела актуальны и рекомендованы для новой разработки. Устаревших методов внутри `imbot.v2` и `im.v2` нет.

Если у вас есть бот на предыдущем поколении бот-платформы, перенесите его на `imbot.v2`. Порядок перехода и таблица соответствия методов — [Миграция с imbot на imbot.v2](./migration.md).
...
Новые возможности, исправления и breaking changes, которые влияют на совместимость интеграций, собраны на странице [Журнал изменений API imbot.v2](./change-log.md). Если формат вызова или ответа метода меняется, предыдущий вариант продолжает поддерживаться шесть месяцев с момента публикации изменения.
...
**[Интерфейс чата](./imbot.v2/ui/index.md)**
