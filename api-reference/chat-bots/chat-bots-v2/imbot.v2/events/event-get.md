...
При `withUserEvents: true` метод возвращает в одном ответе бот-события `ONIMBOTV2*` и пользовательские события `ONIMV2*`.
...
- Python
    ...
    try:
        bitrix_response = client.imbot.v2.event.get(
            bot_id=456,
            offset=1000,
            limit=50,
            with_user_events=True,
        ).response
...
|| **result.nextOffset**
[`integer`](../../../../data-types.md) | Значение для следующего вызова `offset`. Подтверждает уже обработанные события и сдвигает курсор очереди ||
|| **result.hasMore**
