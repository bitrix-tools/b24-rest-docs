# Добавить менеджеров чата imbot.v2.Chat.Manager.add
...
Пользователи, не являющиеся участниками чата, будут проигнорированы без ошибки.
...
- Python
    ...
    try:
        bitrix_response = client.imbot.v2.chat.manager.add(
            bot_id=456,
            dialog_id="chat5",
            user_ids=[
                4,
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imbot.v2.Chat.Manager.add", b24.Params{
    	"botId":    456,
    ...
    if err != nil {
    	return fmt.Errorf("imbot.v2.Chat.Manager.add: %w", err)
    }
...
|| **result.result**
[`boolean`](../../../../data-types.md) | `true` при успешном добавлении менеджеров ||
|| **time**
