# Удалить участника из чата imbot.v2.Chat.User.delete
...
Если пользователь не является участником чата, метод вернет `true` (идемпотентное поведение).
...
- Python
    ...
    try:
        bitrix_response = client.imbot.v2.chat.user.delete(
            bot_id=456,
            dialog_id="chat5",
            user_id=1,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imbot.v2.Chat.User.delete", b24.Params{
    	"botId":    456,
    ...
    if err != nil {
    	return fmt.Errorf("imbot.v2.Chat.User.delete: %w", err)
    }
