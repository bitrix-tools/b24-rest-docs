# Управление полем ввода текста imbot.v2.Chat.TextField.enabled
...
Метод `imbot.v2.Chat.TextField.enabled` включает или отключает поле ввода текста в чате.

Метод работает и в групповых, и в личных чатах с ботом. Доступ проверяется по членству бота в чате, поэтому `ACCESS_DENIED` возвращается только если бот не является участником указанного диалога.
...
- Python
    ...
    try:
        bitrix_response = client.imbot.v2.chat.text_field.enabled(
            bot_id=456,
            dialog_id="chat5",
            enabled=False,
        ).response
...
|| **result.result**
[`boolean`](../../../../data-types.md) | `true` при успешном обновлении настройки ||
|| **time**
