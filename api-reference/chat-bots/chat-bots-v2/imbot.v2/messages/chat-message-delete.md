# Удалить сообщение imbot.v2.Chat.Message.delete
...
Метод `imbot.v2.Chat.Message.delete` удаляет сообщение. Бот может удалить свои сообщения. Если бот является администратором чата — может удалить любое сообщение.

Сообщения, отправленные с параметром `system: true`, имеют `authorId = 0`. Бот не сможет удалить такое сообщение, если не является администратором чата.
...
|| **messageId***
[`integer`](../../../../data-types.md) | ID удаляемого сообщения ||
|| **complete**
[`boolean`](../../../../data-types.md) | Полное удаление из базы данных. Допустимые значения: `true`, `false`. По умолчанию `false` (мягкое удаление) ||
|#
...
- Python
    ...
    try:
        bitrix_response = client.imbot.v2.chat.message.delete(
            bot_id=456,
            message_id=789,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imbot.v2.Chat.Message.delete", b24.Params{
    	"botId":     456,
    	"botToken":  "my_bot_token",
    	"messageId": 789,
    })
    if err != nil {
    	return fmt.Errorf("imbot.v2.Chat.Message.delete: %w", err)
    }
...
|| `ACCESS_DENIED` | Access denied | Сообщение не принадлежит боту и бот не является администратором чата, либо сообщение не существует ||
|#
