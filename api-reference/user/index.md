...
Профиль пользователя — это ключевой объект Битрикс24. Без профиля сотрудник не сможет работать в Битрикс24 и CRM. По идентификатору пользователя можно:
...
- [user.current](./user-current.md), чтобы получить профиль пользователя, от имени которого выполняется запрос,
- [user.get](./user-get.md), чтобы получить список сотрудников по фильтру и узнать `ID` нужного пользователя,
- [user.search](./user-search.md), чтобы найти пользователя по персональным данным с ускоренным поиском,
- [user.fields](./user-fields.md), чтобы получить список полей профиля,
- [user.add](./user-add.md), чтобы пригласить новых пользователей,
- [user.update](./user-update.md), чтобы редактировать профили существующих пользователей, уволить или принять обратно сотрудника.
...
В профиль сотрудника можно добавить собственные поля, например список навыков или дату приема на работу. Полями управляют методы [user.userfield.*](./userfields/index.md) — они работают в скоупе `user.userfield`, выполнять их может только администратор.

- Код нового поля дополняется префиксом `UF_USR_`, например `UF_USR_SKILLS`.
- Значение поля записывают методом [user.update](./user-update.md) — передайте код поля вместе с `ID` пользователя.
- Значения возвращают методы [user.get](./user-get.md) и [user.search](./user-search.md). Чтобы получить все пользовательские поля, передайте в параметре `select` маску `UF_*`.
...
**Почта.** Методы [mail.*](../mail/index.md) работают с письмами и почтовыми ящиками, доступными текущему пользователю. Сотрудников-получателей можно найти методом [mail.recipient.listemployees](../mail/recipient/mail-recipient-listemployees.md). Практические сценарии собраны в разделе [Почта: типовые сценарии](../../tutorials/mail/index.md).
...
Экстранет-пользователи — это внешние участники с ограниченным доступом к Битрикс24. CRM для них недоступна, но доступны чаты, группы и календарь.
...
В профиль пользователя можно встроить приложение — тогда с ним получится работать, не покидая карточку сотрудника.
...
Чтобы обеспечить безопасность данных сотрудников, для приложений и вебхуков доступны три версии скоупа `user`. Версия определяет набор полей профиля, которые вернут методы.

- `user_brief` дает доступ к именам, должностям, фото и другим данным без e-mail, телефонов и почтовых адресов.
- `user_basic` дополнительно открывает контактные данные сотрудников.
- `user` дает доступ ко всем полям профиля, позволяет приглашать новых сотрудников и изменять данные существующих.

{% note tip " " %}

- [Версии скоупа user](user-scope.md)
...
Методы для работы с пользовательскими полями профиля собраны в подразделе [Пользовательские поля](./userfields/index.md).
