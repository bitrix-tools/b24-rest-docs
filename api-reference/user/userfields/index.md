...
В Битрикс24 можно настраивать пользовательские поля для сотрудников. Это позволяет дополнять стандартную информацию о пользователях — например, добавить в профиль список навыков или дату приема на работу.

> Быстрый переход: [все методы](#all-methods)
>
> Пользовательская документация: [Как добавить пользовательские поля в профиль сотрудника](https://helpdesk.bitrix24.ru/open/25548816/)

## Как работать с пользовательскими полями

1. Создайте поле методом [user.userfield.add](./user-userfield-add.md). В параметре `FIELD_NAME` передайте код поля — Битрикс24 дополнит его префиксом `UF_USR_`, например `UF_USR_SKILLS`.
2. Проверьте список созданных полей и их коды методом [user.userfield.list](./user-userfield-list.md).
3. Запишите значение поля в профиль сотрудника методом [user.update](../../user/user-update.md) — передайте код поля вместе с `ID` пользователя.
4. Прочитайте значения методами [user.get](../../user/user-get.md) и [user.search](../../user/user-search.md). Чтобы получить все пользовательские поля, передайте в параметре `select` маску `UF_*`.
5. Измените настройки или название поля методом [user.userfield.update](./user-userfield-update.md), а ненужное поле удалите методом [user.userfield.delete](./user-userfield-delete.md).

Методы работы с профилем сотрудника описаны в разделе [Пользователи](../index.md).

{% note warning "" %}

Методы `user.userfield.*` работают в скоупе `user.userfield`, выполнять их может только администратор. Приложению с ограниченными скоупами `user_brief` или `user_basic` скоуп `user.userfield` нужен и для того, чтобы читать значения пользовательских полей.
...
**Файлы.** Поле типа Файл позволяет загружать и хранить файлы. Файлы не получают идентификаторы в модуле Диск, поэтому методы Диска неприменимы. Работать с файлами можно только через интерфейс Битрикс24.
