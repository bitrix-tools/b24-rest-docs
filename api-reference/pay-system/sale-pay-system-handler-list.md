...
- Python
    ...
    try:
        bitrix_response = client.sale.paysystem.handler.list(
            start=0,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "sale.paysystem.handler.list", nil, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("sale.paysystem.handler.list: %w", err)
    }
