...
- Python
    ...
    try:
        bitrix_response = client.sale.paysystem.settings.payment.get(
            payment_id=10,
            pay_system_id=11,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "sale.paysystem.settings.payment.get", b24.Params{
    	"PAYMENT_ID":    10,
    	"PAY_SYSTEM_ID": 11,
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("sale.paysystem.settings.payment.get: %w", err)
    }
