...
Go SDK `b24gosdk` вызывает методы REST 3.0: передайте базовый адрес с сегментом `/rest/api/`, больше ничего настраивать не нужно. Постраничный обход и `batch` в нем работают только со старой версией — на REST 3.0 вызывайте их обычным вызовом метода.
