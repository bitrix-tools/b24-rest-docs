...
Go SDK [b24gosdk](../sdk/b24gosdk/index.md) вызывает методы REST 3.0: передайте базовый адрес с сегментом `/rest/api/`, больше ничего настраивать не нужно. Постраничный обход и `batch` в нем работают только со старой версией — на REST 3.0 вызывайте их обычным вызовом метода.

Остальные SDK пока не поддерживают в вызовах адрес `/rest/api/`. Используйте прямые HTTP-запросы, например, через `curl`, `fetch`.
...
|| `BITRIX_REST_V3_EXCEPTION_IDEMPOTENCYKEYREUSEDEXCEPTION` | Ключ уже использовался с другим телом запроса | Возьмите новый ключ. Прежний ключ отправляйте только при повторе того же вызова ||
|| `BITRIX_REST_V3_EXCEPTION_INVALIDIDEMPOTENCYKEYEXCEPTION` | Значение ключа не подходит по длине или содержит недопустимые символы | Передайте строку от 1 до 255 печатных символов ASCII ||
|#
