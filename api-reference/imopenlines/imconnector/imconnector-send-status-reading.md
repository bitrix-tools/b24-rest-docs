...
Метод `imconnector.send.status.reading` подтверждает в Битрикс24, что исходящее сообщение открытой линии прочитано во внешней системе. Метод не отправляет сообщение повторно, а только фиксирует результат прочтения.
...
|| **message_id**
[`integer`](../../data-types.md) | Идентификатор сообщения в Битрикс24, для которого нужно установить статус «прочитано» ||
|#
...
Обычно эти значения сохраняют при обработке события и потом используют для отправки статуса прочтения.
...
`тип` | **Описание** ||
|| **id**
[`array`](../../data-types.md) | Массив внешних идентификаторов сообщений, для которых передается статус прочтения. Даже для одного сообщения передавайте массив, например, `["ext-msg-1007"]` ||
|#
...
`тип` | **Описание** ||
|| **SUCCESS**
[`boolean`](../../data-types.md) | Возвращает `true`, если статус прочтения успешно принят и обновлен в Битрикс24 ||
|| **DATA**
