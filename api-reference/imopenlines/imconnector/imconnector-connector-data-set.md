...
Метод `imconnector.connector.data.set` сохраняет в Битрикс24 настройки пользовательского коннектора на указанной открытой линии: идентификатор внешнего канала, ссылки на него и название.

Настройки попадают в интерфейс оператора только после того, как коннектор включен на этой линии: метод [imconnector.status](./imconnector-status.md) должен вернуть `STATUS: true`.

Когда коннектор выключают вызовом [imconnector.activate](./imconnector-activate.md) с параметром `ACTIVE: 0`, сохраненные настройки удаляются вместе с записью статуса.
...
|| **DATA***
[`object`](../../data-types.md) | Объект настроек канала во внешней системе.
...
Все поля объекта необязательные. Метод не перезаписывает настройки целиком: переданные поля обновляются, а непереданные сохраняют прежние значения.

Пустая строка игнорируется, поэтому очистить уже сохраненное значение методом нельзя.
...
Идентификатор нужно взять из внешней системы. Если готового нет, используйте уникальный ключ, например, комбинацию кода коннектора и идентификатора линии: `myconnector_line107` ||
|| **URL**
[`string`](../../data-types.md) | Полная ссылка на чат или канал во внешней системе ||
|| **URL_IM**
[`string`](../../data-types.md) | Ссылка на чат, которую оператор открывает из интерфейса открытых линий. Если отдельной ссылки для оператора нет, поле можно не передавать — у коннектора останется только `URL` ||
|| **NAME**
[`string`](../../data-types.md) | Название канала, которое видит оператор ||
|#
...
`тип` | **Описание** ||
|| **result**
[`boolean`](../../data-types.md) | Всегда `true`, если переданы обязательные параметры: существование коннектора и открытой линии метод не проверяет ||
|| **time**
...
|| `400` | `ERROR_ARGUMENT` | Argument 'DATA' is null or empty | Не передан ключ `DATA`. Пустой объект `DATA` ошибку не вызывает ||
|#
