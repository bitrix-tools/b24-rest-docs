...
- Python
    ...
    try:
        bitrix_response = client.imopenlines.session.mode.pin_all().response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imopenlines.session.mode.pinAll", nil)
    if err != nil {
    	return fmt.Errorf("imopenlines.session.mode.pinAll: %w", err)
    }
