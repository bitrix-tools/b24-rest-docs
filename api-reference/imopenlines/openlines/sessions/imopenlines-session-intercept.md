...
- Python
    ...
    try:
        bitrix_response = client.imopenlines.session.intercept(
            chat_id=2043,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imopenlines.session.intercept", b24.Params{
    	"CHAT_ID": 2043,
    })
    if err != nil {
    	return fmt.Errorf("imopenlines.session.intercept: %w", err)
    }
