...
- Python
    ...
    try:
        bitrix_response = client.imopenlines.session.join(
            chat_id=2043,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imopenlines.session.join", b24.Params{
    	"CHAT_ID": 2043,
    })
    if err != nil {
    	return fmt.Errorf("imopenlines.session.join: %w", err)
    }
