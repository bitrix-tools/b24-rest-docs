...
Метод `imopenlines.session.start` запускает новую сессию в текущем чате открытой линии.
...
- Python
    ...
    try:
        bitrix_response = client.imopenlines.session.start(
            chat_id=2043,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imopenlines.session.start", b24.Params{
    	"CHAT_ID": 2043,
    })
    if err != nil {
    	return fmt.Errorf("imopenlines.session.start: %w", err)
    }
...
```json
{
    ...
    "time": {
        ...
        "processing": 0,
        "date_start": "2026-03-16T14:31:58+03:00",
        "date_finish": "2026-03-16T14:31:58+03:00",
...
`тип` | **Описание** ||
|| **result**
[`boolean`](../../../data-types.md) | Возвращает `true`, если новая сессия успешно запущена ||
|| **time**
