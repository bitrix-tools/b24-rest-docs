...
- Python
    ...
    try:
        bitrix_response = client.imopenlines.bot.session.operator(
            chat_id=112,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imopenlines.bot.session.operator", b24.Params{
    	"CHAT_ID": 112,
    })
    if err != nil {
    	return fmt.Errorf("imopenlines.bot.session.operator: %w", err)
    }
