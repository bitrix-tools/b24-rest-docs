...
Метод `imopenlines.operator.finish` завершает диалог открытой линии от имени текущего оператора.
...
- Python
    ...
    try:
        bitrix_response = client.imopenlines.operator.finish(
            chat_id=2043,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imopenlines.operator.finish", b24.Params{
    	"CHAT_ID": 2043,
    })
    if err != nil {
    	return fmt.Errorf("imopenlines.operator.finish: %w", err)
    }
