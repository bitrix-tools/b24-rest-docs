...
Метод `imopenlines.operator.skip` передает текущий диалог следующему оператору в очереди.
...
- Python
    ...
    try:
        bitrix_response = client.imopenlines.operator.skip(
            chat_id=2043,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imopenlines.operator.skip", b24.Params{
    	"CHAT_ID": 2043,
    })
    if err != nil {
    	return fmt.Errorf("imopenlines.operator.skip: %w", err)
    }
...
`тип` | **Описание** ||
|| **result**
[`boolean`](../../../data-types.md) | Возвращает `true`, если диалог успешно передан дальше по очереди ||
|| **time**
...
|| `400` | `ACCESS_DENIED_KICK` | Недостаточно прав на исключение участников из чата | Указанный чат невозможно передать другому оператору, например, по причине того, что диалог закрыт ||
