...
Метод `imopenlines.operator.another.finish` завершает диалог открытой линии, даже если диалог закреплен за другим оператором.
...
- Python
    ...
    try:
        bitrix_response = client.imopenlines.operator.another.finish(
            chat_id=2043,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "imopenlines.operator.another.finish", b24.Params{
    	"CHAT_ID": 2043,
    })
    if err != nil {
    	return fmt.Errorf("imopenlines.operator.another.finish: %w", err)
    }
...
`тип` | **Описание** ||
|| **result**
[`boolean`](../../../data-types.md) | Возвращает `true`, если диалог успешно завершен ||
|| **time**
