...
- `SONET_GROUP` — системный чат группы или проекта
...
## Особенности поиска чатов групп и проектов

Метод рассчитан на поиск по уникальной паре `ENTITY_TYPE` и `ENTITY_ID`. Если в Битрикс24 есть несколько чатов с одинаковой парой значений, метод вернет один из найденных чатов.

Чтобы получить идентификатор штатного чата группы или проекта, передайте `ENTITY_TYPE` со значением `SONET_GROUP` и `ENTITY_ID` со значением идентификатора группы.

Если для группы создан дубль системного чата через [im.chat.add](./im-chat-add.md), поиск становится неоднозначным: `im.chat.get` может вернуть штатный чат, а `im.dialog.get` с `DIALOG_ID=sg<идентификатор группы>` — другой чат.
...
- Python
    ...
    try:
        bitrix_response = client.im.chat.get(
            entity_type="CRM",
            entity_id="DEAL|1663",
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "im.chat.get", b24.Params{
    	"ENTITY_TYPE": "CRM",
    	"ENTITY_ID":   "DEAL|1663",
    }, b24.WithIdempotent())
    if err != nil {
