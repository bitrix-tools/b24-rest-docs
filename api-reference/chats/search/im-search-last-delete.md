...
Метод разработан для предыдущей версии чата и оставлен только для поддержки существующих интеграций. В текущей версии чата М1 он работает, но результат не отображается в интерфейсе. Замены в текущей версии нет.

{% endnote %}

Метод `im.search.last.delete` удаляет диалог из истории последнего поиска.
...
- Python
    ...
    try:
        bitrix_response = client.im.search.last.delete(
            dialog_id="chat17",
        ).response
...
`тип` | **Описание** ||
|| **result**
[`boolean`](../../data-types.md) | `true`, если запись удалена, иначе `false` ||
|| **time**
