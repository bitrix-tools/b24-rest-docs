# Выключить автоматический статус «Отошел» im.user.status.idle.end
...
Метод `im.user.status.idle.end` выключает автоматический статус «Отошел» для текущего пользователя.
...
- Python
    ...
    try:
        bitrix_response = client.im.user.status.idle.end().response
