...
- `N` — нет ||
|| **ONLY_OPENLINES**
[`string`](../data-types.md) | Возвращать только чаты открытых линий.
...
- Для перехода на следующую страницу увеличивайте `OFFSET` на значение `LIMIT` (`0`, `50`, `100`), а не на количество элементов в ответе.
...
- Если чаты открытых линий не нужны, передавайте `SKIP_OPENLINES = Y` — это снижает вероятность пересечений между страницами.

- Если список небольшой, запрашивайте его одним вызовом с увеличенным `LIMIT`, максимальное значение `200`.
