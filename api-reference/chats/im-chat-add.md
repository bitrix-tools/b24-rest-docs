...
Обязательных параметров у метода нет: без параметров он создает закрытый чат с автоматическим названием, где текущий пользователь — единственный участник и владелец.
...
## Особенности работы с чатами групп и проектов

Для системного чата группы или проекта используется пара параметров:
...
В `ENTITY_TYPE` передается строковое значение `SONET_GROUP`. В `ENTITY_ID` передается идентификатор группы или проекта.

Не передавайте эту пару в `im.chat.add` с идентификатором существующей группы или проекта, чтобы создать дополнительный чат внутри группы. Такой вызов может создать второй системный чат с той же привязкой.

Дубль системного чата может нарушить работу чатов задач в этой группе:

- `tasks.task.add` создает задачу без `chatId` и может вернуть ошибку `ERROR_UNKNOWN_ADD_TASK_ERROR: There was an error while saving task chat`
- `tasks.task.update` и `tasks.task.delete` для такой задачи могут вернуть ошибку `0: There was an error while creating new task chat`

Чаты с привязкой `SONET_GROUP` используются как штатные чаты групп и проектов. Их нельзя переименовать, удалить, покинуть или перепривязать стандартными методами управления чатами.
