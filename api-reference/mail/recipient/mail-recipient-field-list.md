# Получить список полей получателя mail.recipient.field.list
...
Метод `mail.recipient.field.list` возвращает список доступных полей получателя.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/mail.recipient.field.list`
...
- Python
    ...
    try:
        bitrix_response = client.mail.recipient.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "mail.recipient.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("mail.recipient.field.list: %w", err)
    }
