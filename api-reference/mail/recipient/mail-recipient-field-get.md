# Получить поле получателя mail.recipient.field.get
...
Метод `mail.recipient.field.get` возвращает описание поля получателя по имени.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/mail.recipient.field.get`
...
- Python
    ...
    try:
        bitrix_response = client.mail.recipient.field.get(
            name='email',
            select=select,
        ).response
