# Создать дело CRM из письма mail.message.createcrmactivity
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/mail.message.createcrmactivity`
...
- Python
    ...
    try:
        bitrix_response = client.mail.message.createcrmactivity(
            message_id=15,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "mail.message.createcrmactivity", b24.Params{
    	"messageId": 15,
    })
    if err != nil {
...
|| `-` | CRM-модуль не установлен | Установите и настройте модуль CRM ||
...
|| `messageId` | Please try adding the email to the CRM a little later | Проверьте права доступа к CRM и повторите запрос позже ||
|#
