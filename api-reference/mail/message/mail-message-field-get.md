...
- Python
    ...
    try:
        bitrix_response = client.mail.message.field.get(
            name='subject',
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "mail.message.field.get", b24.Params{
    	"name":   "subject",
    	"select": []string{"name", "type", "title"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("mail.message.field.get: %w", err)
    }
