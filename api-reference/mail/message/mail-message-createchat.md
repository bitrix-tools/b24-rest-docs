...
- Python
    ...
    try:
        bitrix_response = client.mail.message.createchat(
            message_id=15,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "mail.message.createchat", b24.Params{
    	"messageId": 15,
    })
    if err != nil {
    	return fmt.Errorf("mail.message.createchat: %w", err)
    }
    ...
    fmt.Println(item.Success, item.ChatID)
    ```
