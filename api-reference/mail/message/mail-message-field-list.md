# Получить список полей письма mail.message.field.list
...
Метод `mail.message.field.list` возвращает список доступных полей письма.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/mail.message.field.list`
...
- Python
    ...
    try:
        bitrix_response = client.mail.message.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "mail.message.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("mail.message.field.list: %w", err)
    }
