# Получить поле почтового ящика mail.mailbox.field.get
...
Метод `mail.mailbox.field.get` возвращает описание поля почтового ящика по имени.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/mail.mailbox.field.get`
...
- Python
    ...
    try:
        bitrix_response = client.mail.mailbox.field.get(
            name='email',
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "mail.mailbox.field.get", b24.Params{
    	"name":   "email",
    	"select": []string{"name", "type", "title"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("mail.mailbox.field.get: %w", err)
    }
