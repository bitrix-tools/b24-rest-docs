# Получить список полей почтового ящика mail.mailbox.field.list
...
Метод `mail.mailbox.field.list` возвращает список доступных полей почтового ящика.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/mail.mailbox.field.list`
...
- Python
    ...
    try:
        bitrix_response = client.mail.mailbox.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "mail.mailbox.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("mail.mailbox.field.list: %w", err)
    }
