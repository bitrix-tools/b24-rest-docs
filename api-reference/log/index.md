# Лента новостей: обзор методов и событий
...
Лента новостей в Битрикс24 помогает сотрудникам обмениваться информацией и следить за событиями компании. В ней отображаются сообщения, задачи, опросы, поздравления, события календаря и запущенные бизнес-процессы.

> Быстрый переход: [все методы и события](#all-methods)
>
> Пользовательская документация: [Что такое Лента в Битрикс24](https://helpdesk.bitrix24.ru/open/18634548/)
...
- [Как запустить бизнес-процесс в ленте](https://helpdesk.bitrix24.ru/open/21290196/)
- [Как настроить бизнес-процессы в ленте новостей](https://helpdesk.bitrix24.ru/open/21659356/)
...
**Сообщения.** Методы `log.blogpost.*` добавляют, изменяют, получают, распространяют и удаляют сообщения Ленты новостей. Создать сообщение можно методом [log.blogpost.add](./log-blogpost-add.md), изменить — методом [log.blogpost.update](./log-blogpost-update.md), удалить — методом [log.blogpost.delete](./log-blogpost-delete.md).

**Комментарии.** Метод [log.blogcomment.add](./blogcomment/log-blogcomment-add.md) добавляет комментарии к сообщениям, метод [log.blogcomment.delete](./blogcomment/log-blogcomment-delete.md) удаляет комментарии. Метод [log.blogcomment.user.get](./blogcomment/log-blogcomment-user-get.md) возвращает комментарии к сообщениям и может фильтровать их по диапазону ID (`FIRST_ID`, `LAST_ID`). Если не передать параметры фильтрации, метод вернет все доступные текущему пользователю комментарии с учетом его прав доступа.
...
- Методы чтения возвращают только сообщения и комментарии, доступные текущему пользователю. Параметры `POST_ID`, `LOG_RIGHTS` и другие фильтры не позволяют обойти права доступа
- Изменить, удалить сообщение или добавить к нему новых получателей может автор сообщения или администратор. Для чужого сообщения без необходимых прав метод вернет ошибку
- Удалить комментарий может его автор или пользователь с полными правами на запись
- Адресаты сообщений задаются кодами пользователей, подразделений и рабочих групп. Метод отклонит запрос, если список адресатов имеет неверный формат или у пользователя нет доступа к указанным получателям
- Методы чтения возвращают опубликованные сообщения Ленты новостей. Удаленные, неопубликованные или недоступные сообщения не попадают в результат

## Обзор методов и событий {#all-methods}

> Scope: [`log`](../scopes/permissions.md)
>
> Кто может выполнять методы и подписываться на события: любой пользователь
...
- События
    ...
    #|
    || **Событие** | **Вызывается** ||
    || [OnLiveFeedPostAdd](./events/on-live-feed-post-add.md) | При добавлении сообщения в Ленту новостей ||
    || [OnLiveFeedPostDelete](./events/on-live-feed-post-delete.md) | При удалении сообщения из Ленты новостей ||
    || [OnLiveFeedPostUpdate](./events/on-live-feed-post-update.md) | При редактировании сообщения в Ленте новостей ||
    |#
