...
- расширенная модель прав на методы,
...
Чтобы узнать, какие связи и поля поддерживаются, используйте [OpenAPI-документацию](#openapi).
...
Права доступа регулируют, что может сделать вебхук или приложение: получить данные задач, создать сделку, удалить бронирование и так далее.

В предыдущей версии можно ограничить права доступа только до модуля. Например, вебхук со `scope: crm` может создавать, изменять, получать или удалять данные, но только для объектов CRM. Без доступа к `scope: tasks` доступ к таким же операциям с задачами будет запрещен.

В новой версии REST 3.0 можно будет ограничивать доступ до конкретного метода. Например, давать доступ только на создание или получение и до отдельных полей в рамках выполнения метода.

Чтобы посмотреть, какие методы доступны в формате scope, вызовите метод `rest.scope.list` без параметров.

```bash
`curl -X POST "https://{portal}/rest/api/{user_id}/{webhook}/rest.scope.list"`
```

{% note info "" %}

Интерфейс назначения расширенных прав для вебхуков и приложений находится в разработке.
...
|| Права доступа, в разработке | На все методы модуля | На конкретные методы и поля ||
