...
В [REST 3.0](../index.md) методы задач получили обновленную архитектуру, унифицированный формат ответов и поддержку связей между объектами. Сейчас REST 3.0 охватывает базовые операции над задачами, работу с чатом задачи, прикрепление файлов, результаты задачи и методы получения схемы полей (`*.field.list` / `*.field.get`). Все, что связано с чек-листами, временем и историей, доступно в REST v2.
...
**Результаты задачи.** Результат фиксирует итог работы по задаче. Для работы с результатами используйте методы раздела [Результаты задачи в REST 3.0](./result/index.md). Они позволяют добавить результат вручную, создать его из сообщения чата задачи, изменить текст результата, получить список результатов и удалить ненужный результат.

**CRM-объекты.** К задаче можно привязать объекты CRM: лиды, контакты, компании, сделки, счета и смарт-процессы. Идентификаторы таких объектов передаются в поле `crmItemIds` в формате с префиксом, например `C_3` для контакта. Данные об связанных объектах CRM недоступны напрямую — их следует запрашивать отдельно через [методы CRM](../../crm/index.md).
...
|| [tasks.task.result.list](./result/tasks-task-result-list.md) | Возвращает список результатов задачи ||
