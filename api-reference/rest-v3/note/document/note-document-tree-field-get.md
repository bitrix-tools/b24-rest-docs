# Получить поле дерева документов note.document.tree.field.get
...
Метод `note.document.tree.field.get` возвращает описание поля дерева документов по имени.
...
- `children` — дочерние страницы документа ||
|| **select**
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/note.document.tree.field.get`
