...
Метод возвращает только первую страницу результатов: параметр `pagination.limit` регулирует размер выдачи, поле `hasMore` в ответе показывает, есть ли дополнительные совпадения, но курсора на следующую страницу нет. Глубокий проход по результатам в REST 3.0 не поддерживается. Чтобы получить больше совпадений, уточняйте `query` или увеличивайте `limit`.
...
Поле отсутствует, если документ доступен через прямой доступ к документу ||
|| **title**
