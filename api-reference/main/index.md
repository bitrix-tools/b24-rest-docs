# Виджеты и Главная страница

Главная — это страница *Битрикс24*, которая заменяет привычную [Ленту новостей](../log/index.md) и позволяет создавать более персонализированный интерфейс (подробности в [статье](https://helpdesk.bitrix24.ru/open/22089066/)).
...
**Виджет** — динамический элемент Главной страницы, который отображает данные, полученные с помощью соответствующего серверного обработчика. Это позволяет делать виджеты интерактивными, отображая изменяющиеся данные в ответ на действия пользователя.
...
Кроме того, Главную страницу, созданную на вашем *Битрикс24*, можно экспортировать в виде готового решения для публикации в [Маркет](../../market/index.md).
...
В виджетах для Главной страницы мы поддерживаем следующие директивы и конструкции Vue:
...
- `openApplication(?params)` — открывает слайдер вашего приложения с возможностью передачи произвольных параметров `params`. Фактически, это способ вызвать [openApplication](../widgets/bx24-widget-methods.md) из интерфейса виджета Главной страницы;
- `openPath(url)` — открывает в слайдере страницу *Битрикс24* по указанному `url`. Фактически, это способ вызвать [openPath](../widgets/bx24-widget-methods.md) из интерфейса виджета Главной страницы.
...
1. Когда администратор Битрикс24 добавляет ваш виджет на Главную страницу или когда обычный пользователь открывает уже опубликованную Главную страницу на своем портале, Битрикс24 делает запрос к URL-обработчика виджета, указанного вами при регистрации виджета.
...
## Примеры синтаксиса Vue для виджетов Главной страницы
