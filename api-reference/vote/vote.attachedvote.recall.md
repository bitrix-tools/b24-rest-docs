...
Метод `vote.AttachedVote.recall` отзывает голос пользователя в активном голосовании. Метод работает только с теми голосованиями, в которых активна опция Разрешить переголосование.
...
Цель идентификатора — защита от дублирования. Если из-за сбоя сети запрос отправится повторно с тем же `actionUuid`, сервер поймет это и отзовет голос только один раз ||
|#
