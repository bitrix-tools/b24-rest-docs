...
Файл попадает в Битрикс24 двумя основными путями: его передают строкой в формате Base64 прямо в поле метода или загружают на Диск и передают в поле `ID` объекта на Диске. Для больших файлов у Диска есть третий путь — загрузка по отдельному адресу `UploadUrl`. Путь зависит от типа поля и от метода, который принимает файл. Уже загруженный файл скачивают по ссылке из ответа метода чтения объекта.

Выбор пути определяют тип поля, форма ответа метода, права и ограничения запроса. Формат конкретного запроса, скачивание, обновление и удаление файлов разобраны в отдельных статьях.
...
Примеры ответов и правила скачивания по подписанной ссылке — в статье [Как скачать файлы](./how-to-download-files.md). Ссылки `urlMachine` и `DOWNLOAD_URL` содержат токен авторизации: не публикуйте их и не пишите в логи.
...
5. Запросите объект методом чтения, чтобы получить `ID` файлов и [ссылки на скачивание](./how-to-download-files.md). Эти `ID` понадобятся, когда файлы нужно будет обновить или удалить.
...
|| Скачать файл по ссылке из ответа | [Как скачать файлы](./how-to-download-files.md) ||
