...
- Python
    ...
    try:
        bitrix_response = client.main.eventlog.field.get(
            name='timestampX',
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "main.eventlog.field.get", b24.Params{
    	"name":   "timestampX",
    	"select": []string{"name", "type", "title", "description", "filterable", "sortable", "multiple"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("main.eventlog.field.get: %w", err)
    }
