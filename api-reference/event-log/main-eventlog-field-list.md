...
- Python
    ...
    try:
        bitrix_response = client.main.eventlog.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "main.eventlog.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "description", "filterable", "sortable", "multiple"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("main.eventlog.field.list: %w", err)
    }
