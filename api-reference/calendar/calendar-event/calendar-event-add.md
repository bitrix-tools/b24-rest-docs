...
Параметр обязателен, если `auto_detect_section` передан и не равен `Y` ||
|| **auto_detect_section**
[`string`](../../data-types.md) | Режим автоопределения секции.

Возможные значения:

- `Y` — включить автоопределение секции
- `N` — использовать переданный `section`

Если параметр не передан, метод пытается автоопределить секцию
||
|| **name***
