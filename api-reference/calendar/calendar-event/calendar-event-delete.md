...
- Python
    ...
    try:
        bitrix_response = client.calendar.event.delete(
            bitrix_id=698,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "calendar.event.delete", b24.Params{
    	"id": 698,
    })
    if err != nil {
    	return fmt.Errorf("calendar.event.delete: %w", err)
    }
...
|| Пустое значение | Не задан id события | Не передан обязательный параметр `id` ||
|| Пустое значение | При удалении события произошла ошибка | У пользователя нет прав доступа к календарю, календарь не существует или другая ошибка ||
|#
