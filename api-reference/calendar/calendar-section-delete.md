...
- Python
    ...
    try:
        bitrix_response = client.calendar.section.delete(
            type="user",
            owner_id=2,
            bitrix_id=521,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "calendar.section.delete", b24.Params{
    	"type":    "user",
    	"ownerId": 2,
    	"id":      521,
    })
    if err != nil {
    	return fmt.Errorf("calendar.section.delete: %w", err)
    }
...
|| Пустое значение | Не задан обязательный параметр "type" для метода "calendar.section.delete" | Не передан обязательный параметр `type` ||
|| Пустое значение | Не задан обязательный параметр "ownerId" для метода "calendar.section.delete" | Не передан обязательный параметр `ownerId` и параметр `type` не равен `user` ||
|| Пустое значение | Не задан id секции | Не передан обязательный параметр `id` ||
|| Пустое значение | Доступ запрещен | Календарь с указанным `id` не существует или нет прав для редактирования календаря ||
|| Пустое значение | При удалении секции произошла ошибка | Другая ошибка ||
|#
