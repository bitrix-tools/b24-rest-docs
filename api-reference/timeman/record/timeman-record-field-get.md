...
- Python
    ...
    try:
        bitrix_response = client.timeman.record.field.get(
            name='startTime',
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "timeman.record.field.get", b24.Params{
    	"name":   "startTime",
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("timeman.record.field.get: %w", err)
    }
