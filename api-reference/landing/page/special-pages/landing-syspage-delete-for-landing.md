...
Посмотреть, какие специальные страницы назначены сайту, можно методом [landing.syspage.get](./landing-syspage-get.md). Чтобы снять все привязки сайта, а не одной страницы, используйте метод [landing.syspage.deleteForSite](./landing-syspage-delete-for-site.md).
...
Значение `scope` должно соответствовать типу сайта, которому принадлежит страница [(подробное описание)](../../types.md) ||
|| **id***
...
- Python
    ...
    try:
        bitrix_response = client.landing.syspage.delete_for_landing(
            bitrix_id=8593,
        ).response
...
Значение не подтверждает, что привязки были удалены. Метод может вернуть `true` без изменений, например если страница не найдена, не передан подходящий параметр `scope`, у пользователя нет прав на сайт страницы или для страницы нет специальных привязок ||
|| **time**
