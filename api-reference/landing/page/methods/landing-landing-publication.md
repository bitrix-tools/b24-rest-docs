...
- Python
    ...
    try:
        bitrix_response = client.landing.landing.publication(
            lid=351,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "landing.landing.publication", b24.Params{
    	"lid": 351,
    })
    if err != nil {
    	return fmt.Errorf("landing.landing.publication: %w", err)
    }
...
|| `LANDING_NOT_EXIST` | Страница не найдена: в `lid` передан идентификатор несуществующей, удаленной или недоступной страницы. Для страниц Базы знаний, групп и вайба убедитесь, что передан параметр `scope` ||
