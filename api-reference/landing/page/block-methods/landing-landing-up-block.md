...
Метод `landing.landing.upblock` поднимает блок на одну позицию вверх в черновике страницы. Если страница уже опубликована, для посетителей изменение станет видно после команды «Опубликовать изменения» в интерфейсе или после вызова метода [landing.landing.publication](../methods/landing-landing-publication.md).
...
- Python
    ...
    try:
        bitrix_response = client.landing.landing.upblock(
            lid=351,
            block=6428,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "landing.landing.upblock", b24.Params{
    	"lid":   351,
    	"block": 6428,
    })
    if err != nil {
    	return fmt.Errorf("landing.landing.upblock: %w", err)
    }
...
```json
{
    ...
    "time": {
        ...
        "processing": 1,
        "date_start": "2026-03-20T07:11:12+03:00",
        "date_finish": "2026-03-20T07:11:13+03:00",
...
|| `ACCESS_DENIED` | У пользователя нет прав для вызова метода или нет права редактировать страницу `lid` ||
...
|| `BLOCK_WRONG_SORT` | Блок уже находится вверху страницы, поэтому его нельзя поднять еще выше ||
