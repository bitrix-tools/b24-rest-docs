...
>
> Кто может выполнять метод: администратор или пользователь с правом «полный доступ» к разделу «Сайты и магазины». Если передан параметр `scope`, метод доступен только администратору портала
...
С любым из этих значений переключить модель прав может только администратор портала. Без параметра `scope` метод работает в разделе «Сайты и магазины» ||
|| **mode***
...
|| `IS_NOT_ADMIN` | Для метода нужны права администратора или право «полный доступ» к разделу «Сайты и магазины». Ошибку также вернет вызов с параметром `scope`, если его выполняет не администратор портала ||
