...
Для Базы знаний метод управляет видимостью сайта и его страниц для пользователей. 
...
- Python
    ...
    try:
        bitrix_response = client.landing.site.publication(
            bitrix_id=1688,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "landing.site.publication", b24.Params{
    	"id": 1688,
    })
    if err != nil {
    	return fmt.Errorf("landing.site.publication: %w", err)
    }
