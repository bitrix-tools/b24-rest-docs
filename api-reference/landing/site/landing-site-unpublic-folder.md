...
Метод `landing.site.unPublicFolder` снимает с публикации папку сайта и ее цепочку родительских папок.
...
- Python
    ...
    try:
        bitrix_response = client.landing.site.un_public_folder(
            folder_id=737,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "landing.site.unPublicFolder", b24.Params{
    	"folderId": 737,
    })
    if err != nil {
    	return fmt.Errorf("landing.site.unPublicFolder: %w", err)
    }
...
```json
{
    ...
    "time": {
        ...
        "processing": 0,
        "date_start": "2026-03-12T06:36:07+03:00",
        "date_finish": "2026-03-12T06:36:07+03:00",
...
`тип` | **Описание** ||
|| **result**
[`boolean`](../../data-types.md) | Возвращает `true`, если папка и ее цепочка родительских папок успешно сняты с публикации ||
|| **time**
