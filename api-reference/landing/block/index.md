...
Структура блока описана в статьях про [атрибуты](./attributes.md), [типы нод](./node-types.md), [расширенное описание карточек](./extended-description.md) и [файл манифеста](./manifest.md). Переводы подписей блока для разных языков описаны в статье [Локализация блока](./localization.md).
