...
Метод `telephony.externalCall.searchCrmEntities` возвращает CRM-объекты по номеру телефона клиента и данные ответственного сотрудника. Среди лидов метод возвращает только доступный текущему пользователю активный лид.
...
- Python
    ...
    try:
        bitrix_response = client.telephony.external_call.search_crm_entities(
            phone_number="79062195047",
        ).response
...
Возможные значения:
- `CONTACT` — контакт
- `LEAD` — доступный текущему пользователю активный лид
- `COMPANY` — компания ||
|| **CRM_ENTITY_ID**
