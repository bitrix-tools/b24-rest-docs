# Получить описание поля Follow-up call.followup.field.get
...
`тип` | **Описание** ||
|| **name***
[`string`](../../data-types.md) | Имя поля Follow-up, описание которого нужно получить.

Доступные поля можно получить методом [call.followup.field.list](./call-followup-field-list.md) ||
|| **select**
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/call.followup.field.get`
...
#|
|| **Поле** | **Описание ошибки** | **Как исправить** ||
|| `name` | Поле `#FIELD#` не найдено | Передайте существующее имя поля. Список полей можно получить методом [call.followup.field.list](./call-followup-field-list.md) ||
|#
