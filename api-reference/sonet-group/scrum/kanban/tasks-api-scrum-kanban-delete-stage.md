...
- Python
    ...
    try:
        bitrix_response = client.tasks.api.scrum.kanban.delete_stage(
            stage_id=65,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.api.scrum.kanban.deleteStage", b24.Params{
    	"stageId": 65,
    })
    if err != nil {
    	return fmt.Errorf("tasks.api.scrum.kanban.deleteStage: %w", err)
    }
...
`тип` | **Описание** ||
|| **result**
[`boolean`](../../../data-types.md) | Возвращает `true`, если стадия успешно удалена ||
|| **time**
...
|| `0` | `System error` | Не удалось удалить стадию из-за внутренней ошибки ||
|#
