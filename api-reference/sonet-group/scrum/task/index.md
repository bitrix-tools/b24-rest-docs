...
1. Создайте задачу методом [tasks.task.add](../../../tasks/tasks-task-add.md) или найдите существующую задачу методом [tasks.task.list](../../../tasks/tasks-task-list.md).
2. Получите идентификатор Скрама методом [socialnetwork.api.workgroup.list](../../socialnetwork-api-workgroup-list.md).
3. Привяжите задачу к Скраму и заполните поля Скрама методом [tasks.api.scrum.task.update](./tasks-api-scrum-task-update.md).
4. Проверьте данные задачи Скрама методом [tasks.api.scrum.task.get](./tasks-api-scrum-task-get.md).

## Какие данные возвращает задача Скрама

Метод [tasks.api.scrum.task.get](./tasks-api-scrum-task-get.md) возвращает дополнительные поля задачи для работы в Скраме:

- `entityId` — идентификатор бэклога или спринта
- `storyPoints` — оценка сложности задачи в виде строки
- `epicId` — идентификатор эпика
- `sort` и `sortFloat` — значения сортировки
- `createdBy` и `modifiedBy` — идентификаторы пользователей, которые создали и последними изменили запись задачи в Скраме
...
Перечень объявленных полей и их типы можно получить методом [tasks.api.scrum.task.getFields](./tasks-api-scrum-task-get-fields.md). Поле `groupId` метод `getFields` не перечисляет: контроллер добавляет его непосредственно в ответ `tasks.api.scrum.task.get`.
