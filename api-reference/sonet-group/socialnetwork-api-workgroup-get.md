...
```json
{
    "result": {
        ...
        "KEYWORDS": "тег группы,ещё один тег группы",
        "CLOSED": "N",
        "VISIBLE": "Y",
        "OPENED": "N",
        ...
        "SEARCH_INDEX": "Группа для демонстрации метода Первая строка описания группы\r\nВторая строка описания группы тег группы #тег группы ещё один тег группы #ещё один тег группы group@example.com",
        "LANDING": "N",
        "SCRUM_OWNER_ID": 0,
...
|| **KEYWORDS** [`string`](../data-types.md#standart-types) | Теги группы (не задач группы) через запятую: `тег1,тег2` ||
...
|| **INVITED_MEMBERS** [`array`](../data-types.md#standart-types) | Массив идентификаторов пользователей портала, которым было отправлено приглашение в группу, но они его ещё не приняли ||
...
||
|| **TAGS** [`array`](../data-types.md#standart-types) | Теги группы (не задач группы), аналогично `KEYWORDS`, но в формате массива ||
|| **ACTIONS** [`object`](../data-types.md#standart-types) | Данные о доступных текущему пользователю (т.е. владельцу вебхука или пользователю приложения) операциях над группой:
...
||
|| **USER_DATA** [`object`](../data-types.md#standart-types) | Информация о текущем пользователе (т.е. владельце вебхука или пользователя приложения) относительно группы:
...
|| **IS_PIN** [`boolean`](../data-types.md#standart-types) | Значение `true`/`false` - закреплена ли группа у текущего пользователя (т.е. владельца вебхука или пользователя приложения) на странице групп и проектов ||
...
- `GROUP_MEMBERS_LIST[0][invited]` [`boolean`](../data-types.md#standart-types) - Значение `true`/`false` - пользователь приглашён
...
||
|| **COUNTERS** [`object`](../data-types.md#standart-types) | Счётчики:
...
|| **ADDITIONAL_DATA** [`object`](../data-types.md#standart-types) | Дополнительные данные для текущего пользователя (т.е. владельца вебхука или пользователя приложения):
