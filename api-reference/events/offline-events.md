...
Офлайн-события доступны только приложению: регистрация с `event_type = offline` и методы `event.offline.*` работают в контексте [приложения](../../settings/app-installation/index.md) и требуют прав администратора
...
Метод [event.offline.get](./event-offline-get.md) возвращает записи очереди в массиве `events`, метод [event.offline.list](./event-offline-list.md) — прямо в `result`, с дополнительными полями `PROCESS_ID` и `ERROR`. Состав записи одинаковый для всех событий.
...
|| **TIMESTAMP_X**
[`datetime`](../data-types.md) | Время последнего изменения объекта ||
|| **EVENT_NAME**
[`string`](../data-types.md) | Имя события, например `ONCRMDEALUPDATE` ||
|| **EVENT_DATA**
[`object`](../data-types.md) или [`boolean`](../data-types.md) | Данные события — те же, что приходят в обработчик обычного события. Для событий CRM это идентификатор объекта в `FIELDS.ID`. Если у события нет данных, поле равно `false` ||
|| **EVENT_ADDITIONAL**
[`object`](../data-types.md) или [`boolean`](../data-types.md) | Данные авторизации события. Если изменение сделал пользователь, его идентификатор приходит в поле `user_id`; иначе поле равно `false` ||
|| **MESSAGE_ID**
[`string`](../data-types.md) | Ключ записи — хеш имени события и данных. Повторное событие с тем же ключом обновляет запись, а не добавляет новую: так в очереди остается одна запись на объект. Значение передают в `message_id` методов [event.offline.clear](./event-offline-clear.md) и [event.offline.error](./event-offline-error.md) ||
|| **PROCESS_ID**
[`string`](../data-types.md) | Только в ответе `event.offline.list`. Идентификатор пакета, которым запись зарезервирована через `event.offline.get` с `clear = 0`; пустой, пока запись не зарезервирована ||
|| **ERROR**
[`string`](../data-types.md) | Только в ответе `event.offline.list`. `1` — запись помечена ошибочной методом `event.offline.error`, `0` — нет ||
|#

Пример ответа [event.offline.get](./event-offline-get.md) с резервированием пакета — одна запись об изменении сделки:
...
Поле `process_id` заполнено только при вызове с `clear = 0` или с переданным `process_id`. Полное описание параметров и ошибок — на страницах методов.
...
В основном сценарии очередью управляют шесть параметров. Фильтры, сортировка и постраничная навигация — на страницах методов.

#|
|| **Параметр** | **Метод** | **Что делает** ||
|| `limit` | [event.offline.get](./event-offline-get.md) | Размер пакета записей. По умолчанию `50` ||
|| `clear` | [event.offline.get](./event-offline-get.md) | `1` — удалить записи сразу после выдачи, по умолчанию. `0` — только пометить пакет и вернуть `process_id`; работает только в расширенном режиме ||
|| `process_id` | [event.offline.get](./event-offline-get.md), [event.offline.clear](./event-offline-clear.md), [event.offline.error](./event-offline-error.md) | Идентификатор зарезервированного пакета: повторно получить, подтвердить или пометить ошибочным ||
|| `message_id` | [event.offline.clear](./event-offline-clear.md), [event.offline.error](./event-offline-error.md) | Массив ключей `MESSAGE_ID`, чтобы обработать часть пакета, а не весь ||
|| `auth_connector` | [event.bind](./event-bind.md), [event.offline.get](./event-offline-get.md), [event.offline.list](./event-offline-list.md) | Ключ источника — отдельная очередь, которая не получает изменения от самого приложения ||
|| `error` | [event.offline.get](./event-offline-get.md) | `1` — вернуть записи, помеченные ошибочными методом `event.offline.error`. По умолчанию `0` — только необработанные записи ||
|#
...
- Go
    ...
    // в структуру под форму ответа со страницы метода.
    fmt.Printf("%s\n", res.Result)
    ```
...
Метод [event.offline.get](./event-offline-get.md) поддерживает параллельные запросы: каждый получит свой набор записей, не пересекающийся с другими.

### Регистрация ошибок

Если события не удалось обработать, пометьте их методом [event.offline.error](./event-offline-error.md). Передайте `process_id` и массив `message_id` ошибочных записей. Помеченные записи не попадут в обычную выдачу: чтобы получить их повторно, вызовите [event.offline.get](./event-offline-get.md) с параметром `error = 1`.
