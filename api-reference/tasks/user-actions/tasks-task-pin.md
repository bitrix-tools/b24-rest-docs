# Закрепить задачу tasks.task.pin
...
Метод `tasks.task.pin` закрепляет задачу в списке задач текущего пользователя.
...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.pin(
            bitrix_id=3897,
        ).response
...
|| **task**
[`object`](../../data-types.md) | Объект с [описанием задачи](../fields.md) после закрепления ||
|| **time**
