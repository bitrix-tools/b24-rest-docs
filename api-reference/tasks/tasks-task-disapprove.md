...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.disapprove(
            task_id=8017,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.disapprove", b24.Params{
    	"taskId": 8017,
    })
    if err != nil {
    	return fmt.Errorf("tasks.task.disapprove: %w", err)
    }
