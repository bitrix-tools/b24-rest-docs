...
**Пользователь.** Комментарий имеет привязку к пользователю по числовому идентификатору в параметре `AUTHOR_ID`. Получить идентификатор пользователя можно методом [user.get](../../user/user-get.md).
...
Загрузите файл одним из методов:
...
Получите список файлов одним из методов:
...
Метод [task.commentitem.delete](./task-comment-item-delete.md) удаляет комментарий с результатом.
...
- Методы
    ...
    || [task.commentitem.get](./task-comment-item-get.md) | Возвращает комментарий задачи по `id` ||
...
- События
    ...
    || [OnTaskCommentUpdate](./events-comment/on-task-comment-update.md) | При обновлении комментария в задаче ||
