# Получить список полей задачи v 3.0 tasks.task.field.list
...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "description", "filterable", "sortable", "multiple"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("tasks.task.field.list: %w", err)
    }
