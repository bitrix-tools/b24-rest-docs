...
Метод `tasks.task.file.field.list` возвращает список доступных полей файлов задачи.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/tasks.task.file.field.list`
...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.file.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.file.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "description", "filterable", "sortable", "multiple"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("tasks.task.file.field.list: %w", err)
    }
...
```json
{
    ...
    "time": {
        ...
        "processing": 0,
        "date_start": "2026-03-16T11:47:09+03:00",
        "date_finish": "2026-03-16T11:47:09+03:00",
