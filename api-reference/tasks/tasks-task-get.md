...
Для получения пользовательских полей укажите их в `SELECT`. Узнать названия пользовательских полей можно методом [tasks.task.getFields](./tasks-task-get-fields.md)||
|#

{% note info "" %}

Поле `CHAT_ID`, идентификатор чата для [новой карточки задачи](tasks-new.md), возвращается по умолчанию. Используйте его значение для работы с методами [чата](../chats/index.md)
