...
- Python
    ...
    try:
        bitrix_response = client.tasks.template.checklist.move_after(
            template_id=139,
            check_list_item_id=25,
            after_item_id=29,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.template.checklist.moveAfter", b24.Params{
    	"templateId":      139,
    	"checkListItemId": 25,
    	"afterItemId":     29,
    })
    if err != nil {
    	return fmt.Errorf("tasks.template.checklist.moveAfter: %w", err)
    }
