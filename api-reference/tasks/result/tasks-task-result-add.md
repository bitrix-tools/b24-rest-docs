...
Идентификатор можно получить при [создании задачи](../tasks-task-add-rest-v3.md), [получении задачи](../tasks-task-get-rest-v3.md) или старым методом [получения списка задач](../tasks-task-list-rest-v3.md) ||
|| **text***
...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.result.add(
            fields=fields,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.result.add", b24.Params{
    	"fields": b24.Params{
    		"taskId": 51,
    		"text":   "Работа по задаче выполнена",
    	},
    ...
    if err != nil {
    	return fmt.Errorf("tasks.task.result.add: %w", err)
    }
