...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.result.delete(
            bitrix_id=17,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.result.delete", b24.Params{
    	"id": 17,
    })
    if err != nil {
    	return fmt.Errorf("tasks.task.result.delete: %w", err)
    }
