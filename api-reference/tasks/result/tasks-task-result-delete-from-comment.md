...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.result.delete_from_comment(
            comment_id=3199,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.result.deleteFromComment", b24.Params{
    	"commentId": 3199,
    })
    if err != nil {
    	return fmt.Errorf("tasks.task.result.deleteFromComment: %w", err)
    }
