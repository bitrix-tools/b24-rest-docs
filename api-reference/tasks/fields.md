...
Страница помогает разобрать поля, которые возвращают методы [tasks.task.get](./tasks-task-get.md) и [tasks.task.list](./tasks-task-list.md). Для получения метаданных полей используйте метод [tasks.task.getFields](./tasks-task-get-fields.md).

Выбирайте источник в зависимости от задачи:

- `tasks.task.getFields` — получить типы, допустимые значения и обязательность полей
- `tasks.task.get` — получить одну задачу и вычисляемые данные, доступные текущему пользователю
- `tasks.task.list` — получить несколько задач и указать нужные поля в параметре `select`

Метаданные и значения в ответе решают разные задачи. Например, метаданные могут обозначать идентификатор как `integer`, а значение в JSON-ответе возвращается строкой. Таблицы ниже описывают значения ответа методов получения задач.

Доступность записи и изменения полей зависит от роли пользователя, прав на группу, иерархии, статуса задачи и настроек задачи, например `allowChangeDeadline`. Перед изменением задачи проверяйте объект `action`: его логические поля показывают действия, доступные текущему пользователю.
...
В методах [создания](./tasks-task-add.md) и [обновления](./tasks-task-update.md) задачи, вместе с `startDatePlan` необходимо передавать `endDatePlan` ||
|| **endDatePlan**
[`string`](../data-types.md) | Плановая дата окончания.

В методах [создания](./tasks-task-add.md) и [обновления](./tasks-task-update.md) задачи, вместе с `endDatePlan` необходимо передавать `startDatePlan` ||
|| **guid**
...
|| **subordinate**
[`string`](../data-types.md) | Признак того, что задача является подзадачей. Возможные значения:
- `Y` — да
- `N` — нет ||
|| **exchangeModified**
...
- `5` — завершена
- `6` — отложена
- `7` — отклонена. Статус определен в модели задачи, но может отсутствовать в перечне `values` метода `tasks.task.getFields` ||
|| **statusChangedDate**
...
Значение `monts` закреплено в контракте API. Передавайте его без исправления на `months` ||
|| **favorite**
...
|| **accomplicesData**
[`object`](../data-types.md) | Объект с описанием пользователей — соисполнителей.

Ключ объекта — идентификатор пользователя, а значение — объект с [описанием пользователя](#user).

Если соисполнителей нет, поле может содержать пустой массив `[]` ||
|| **auditorsData**
...
- [Получить метаданные полей задачи](./tasks-task-get-fields.md)
- [Получить задачу](./tasks-task-get.md)
- [Получить список задач](./tasks-task-list.md)
