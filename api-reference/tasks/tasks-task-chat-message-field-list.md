...
Метод `tasks.task.chat.message.field.list` возвращает список доступных полей сообщения чата задачи.
...
`https://{адрес}/rest/api/{id_пользователя}/{токен_вебхука}/tasks.task.chat.message.field.list`
...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.chat.message.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.chat.message.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "description", "filterable", "sortable", "multiple"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("tasks.task.chat.message.field.list: %w", err)
    }
...
```json
{
    ...
    "time": {
        ...
        "processing": 0,
        "date_start": "2026-03-16T11:24:47+03:00",
        "date_finish": "2026-03-16T11:24:47+03:00",
