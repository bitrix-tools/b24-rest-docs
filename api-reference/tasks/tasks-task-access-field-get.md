# Получить поле прав доступа задачи tasks.task.access.field.get
...
Метод `tasks.task.access.field.get` возвращает описание поля прав доступа задачи по имени.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/tasks.task.access.field.get`
...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.access.field.get(
            name='id',
            select=select,
        ).response
