...
Метод `task.item.getdependson` возвращает массив, содержащий идентификаторы задач, от которых зависит задача (опция `Предыдущие задачи` в форме создания задачи).
...
- Go
    ...
    res, err := client.Core().Call(ctx, "task.item.getdependson", b24.Params{
    	"TASKID": 13,
    })
    if err != nil {
    	return fmt.Errorf("task.item.getdependson: %w", err)
    }
