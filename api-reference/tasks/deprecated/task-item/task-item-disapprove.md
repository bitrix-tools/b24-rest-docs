...
Метод `task.item.disapprove` переводит задачу, ожидающую контроля, в статус «не выполняется».

{% note warning "DEPRECATED" %}

Развитие метода остановлено. Используйте [tasks.task.disapprove](../../tasks-task-disapprove.md).
...
- Go
    ...
    res, err := client.Core().Call(ctx, "task.item.disapprove", b24.Params{
    	"TASKID": 13,
    })
    if err != nil {
    	return fmt.Errorf("task.item.disapprove: %w", err)
    }
