...
Метод `task.item.getmanifest` возвращает список методов вида `task.item.*` и их описание.
...
Развитие метода остановлено. Используйте [tasks.task.getFields](../../tasks-task-get-fields.md).
...
- Go
    ...
    res, err := client.Core().Call(ctx, "task.item.getmanifest", nil)
    if err != nil {
    	return fmt.Errorf("task.item.getmanifest: %w", err)
    }
