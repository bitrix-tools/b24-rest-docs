# Получить поле сообщения чата задачи tasks.task.chat.message.field.get
...
Метод `tasks.task.chat.message.field.get` возвращает описание поля сообщения чата задачи по имени.
...
`https://{адрес}/rest/api/{id_пользователя}/{токен_вебхука}/tasks.task.chat.message.field.get`
...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.chat.message.field.get(
            name='taskId',
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.chat.message.field.get", b24.Params{
    	"name":   "taskId",
    	"select": []string{"name", "type", "title", "description", "filterable", "sortable", "multiple"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("tasks.task.chat.message.field.get: %w", err)
    }
