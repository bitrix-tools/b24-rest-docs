...
- Python
    ...
    try:
        bitrix_response = client.task.item.userfield.delete(
            bitrix_id=1325,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "task.item.userfield.delete", b24.Params{
    	"ID": 1325,
    })
    if err != nil {
    	return fmt.Errorf("task.item.userfield.delete: %w", err)
    }
