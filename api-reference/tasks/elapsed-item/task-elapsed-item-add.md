...
>
> Кто может выполнять метод: пользователь с правом учета затраченного времени в задаче
...
|| **ARFIELDS***
[`object`](../../data-types.md) | Поля записи о затраченном времени [(подробное описание)](#arfields) ||
|#

### Параметр ARFIELDS {#arfields}
...
|| **USER_ID**
[`integer`](../../data-types.md) | Идентификатор автора записи. По умолчанию — идентификатор текущего пользователя. Передать идентификатор другого пользователя нельзя ||
|| **CREATED_DATE**
[`datetime`](../../data-types.md) | Дата создания записи. По умолчанию — текущие дата и время ||
|| **DATE_START**
[`datetime`](../../data-types.md) | Дата и время начала учета ||
|| **DATE_STOP**
[`datetime`](../../data-types.md) | Дата и время завершения учета ||
|| **SOURCE**
[`integer`](../../data-types.md) | Источник записи. Метод всегда сохраняет значение `2` — запись добавлена вручную. Переданное значение заменяется на `2` ||
|#
...
|| `ERROR_CORE` | `0x000004` | Действие не разрешено, в том числе при попытке указать другого автора ||
|| `ERROR_CORE` | `0x000008` | Не удалось добавить запись ||
|| `ERROR_CORE` | `0x000100` | Не передан обязательный параметр, указан неверный тип или недопустимое поле ||
|| `ERROR_CORE` | `TASKS_ERROR_ASSERT_EXCEPTION` | В объекте `ARFIELDS` не передано поле `SECONDS` ||
|#
