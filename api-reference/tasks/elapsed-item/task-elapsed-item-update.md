...
>
> Кто может выполнять метод: автор записи или администратор с доступом к задаче
...
|| **ARFIELDS***
[`object`](../../data-types.md) | Изменяемые поля записи [(подробное описание)](#arfields) ||
|#
...
- Python
    ...
    try:
        bitrix_response = client.task.elapseditem.update(
            task_id=691,
            item_id=5,
            arfields=arfields,
        ).response
...
`тип` | **Описание** ||
|| **result**
[`null`](../../data-types.md) | Возвращает `null`, если запись успешно изменена ||
|| **time**
...
|| `ERROR_CORE` | `0x000004` | Действие не разрешено. Изменить запись может ее автор или администратор ||
|| `ERROR_CORE` | `0x000008` | Не удалось изменить запись ||
|| `ERROR_CORE` | `0x000100` | Не передан обязательный параметр, указан неверный тип или передано недопустимое поле ||
