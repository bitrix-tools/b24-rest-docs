...
**Пользователь.** Поток привязан к пользователю по идентификатору администратора потока `ownerId`. Получить идентификатор пользователя можно методом [user.get](../../user/user-get.md). Если при создании потока не указать идентификатор, администратором потока будет его создатель.
...
#|
|| **Метод** | **Описание** ||
|| [tasks.flow.Flow.create](./tasks-flow-flow-create.md) | Создает поток ||
|| [tasks.flow.Flow.update](./tasks-flow-flow-update.md) | Изменяет поток ||
|| [tasks.flow.Flow.get](./tasks-flow-flow-get.md) | Возвращает поток ||
|| [tasks.flow.Flow.delete](./tasks-flow-flow-delete.md) | Удаляет поток ||
|| [tasks.flow.Flow.isExists](./tasks-flow-flow-is-exists.md) | Проверяет, существует ли поток с таким названием ||
|| [tasks.flow.Flow.activate](./tasks-flow-flow-activate.md) | Включает или выключает поток ||
|| [tasks.flow.Flow.pin](./tasks-flow-flow-pin.md) | Закрепляет или открепляет поток в списке ||
|#
