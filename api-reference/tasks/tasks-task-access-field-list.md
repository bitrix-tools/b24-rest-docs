...
- Python
    ...
    try:
        bitrix_response = client.tasks.task.access.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "tasks.task.access.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "description", "filterable", "sortable", "multiple"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("tasks.task.access.field.list: %w", err)
    }
