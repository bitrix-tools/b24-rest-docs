...
- Python
    ...
    try:
        bitrix_response = client.task.planner.getlist().response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "task.planner.getlist", nil, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("task.planner.getlist: %w", err)
    }
