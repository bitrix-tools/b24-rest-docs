# Получить список полей документа note.document.field.list
...
Метод `note.document.field.list` возвращает список доступных полей документа.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/note.document.field.list`
...
- cURL (Webhook)
    ...
    https://**put_your_bitrix24_address**/rest/api/**put_your_user_id_here**/**put_your_webhook_here**/note.document.field.list
    ```
...
- cURL (OAuth)
    ...
    https://**put_your_bitrix24_address**/rest/api/note.document.field.list
    ```
...
- JS (TS)
    ...
    type DocumentFieldListResult = {
      items: DocumentFieldItem[]
    }

    try {
      const response = await $b24.actions.v3.call.make<DocumentFieldListResult>({
      ...
      if (!response.isSuccess) {
        ...
        console.info('Document fields:', result.items)
      }
    } catch (error) {
      console.error(error)
    }
...
- JS (UMD)
    ...
    <script>
      async function listDocumentFields() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', listDocumentFields)
    </script>
    ```
...
- Python
    ...
    try:
        bitrix_response = client.note.document.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "note.document.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("note.document.field.list: %w", err)
    }
