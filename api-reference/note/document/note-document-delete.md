# Переместить документ в корзину note.document.delete
...
Метод `note.document.delete` переносит документ и все его дочерние страницы в корзину.

{% note info "" %}

Восстановить документ из корзины можно только через интерфейс.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/note.document.delete`
...
- Python
    ...
    try:
        bitrix_response = client.note.document.delete(
            bitrix_id=77,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "note.document.delete", b24.Params{
    	"id": 77,
    })
    if err != nil {
    	return fmt.Errorf("note.document.delete: %w", err)
    }
...
`тип` | **Описание** ||
|| **result**
[`object`](../../data-types.md) | Объект с результатом удаления документа ||
|| **result**
[`boolean`](../../data-types.md) | Значение `true`, если документ успешно перенесен в корзину ||
|| **time**
...
#|
|| **Поле** | **Описание ошибки** | **Как исправить** ||
|| `id` | Документ не найден | Проверьте, что документ существует и еще не находится в корзине ||
|#
