# Получить список полей поиска документов note.document.search.field.list
...
Метод `note.document.search.field.list` возвращает список доступных полей результата поиска документов.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/note.document.search.field.list`
...
- cURL (Webhook)
    ...
    https://**put_your_bitrix24_address**/rest/api/**put_your_user_id_here**/**put_your_webhook_here**/note.document.search.field.list
    ```
...
- cURL (OAuth)
    ...
    https://**put_your_bitrix24_address**/rest/api/note.document.search.field.list
    ```
...
- JS (TS)
    ...
    type SearchFieldListResult = {
      items: SearchFieldItem[]
    }

    try {
      const response = await $b24.actions.v3.call.make<SearchFieldListResult>({
      ...
      if (!response.isSuccess) {
        ...
        console.info('Search fields:', result.items)
      }
    } catch (error) {
      console.error(error)
    }
...
- JS (UMD)
    ...
    <script>
      async function listSearchFields() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', listSearchFields)
    </script>
    ```
...
- Python
    ...
    try:
        bitrix_response = client.note.document.search.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "note.document.search.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("note.document.search.field.list: %w", err)
    }
...
- [{#T}](./note-document-search-field-get.md)
