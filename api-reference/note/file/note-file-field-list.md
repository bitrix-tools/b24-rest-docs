# Получить список полей файла документа note.file.field.list
...
Метод `note.file.field.list` возвращает список доступных полей файла документа.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/note.file.field.list`
...
- cURL (Webhook)
    ...
    https://**put_your_bitrix24_address**/rest/api/**put_your_user_id_here**/**put_your_webhook_here**/note.file.field.list
    ```
...
- cURL (OAuth)
    ...
    https://**put_your_bitrix24_address**/rest/api/note.file.field.list
    ```
...
- JS (TS)
    ...
    try {
      ...
      if (!response.isSuccess) {
        ...
        console.info('File fields:', result.items)
      }
    } catch (error) {
      console.error(error)
    }
...
- JS (UMD)
    ...
    <script>
      ...
      document.addEventListener('DOMContentLoaded', listFileFields)
    </script>
    ```
...
- Python
    ...
    try:
        bitrix_response = client.note.file.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "note.file.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("note.file.field.list: %w", err)
    }
