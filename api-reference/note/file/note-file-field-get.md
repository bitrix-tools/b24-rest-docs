# Получить поле файла документа note.file.field.get
...
Метод `note.file.field.get` возвращает описание поля файла документа по имени.
...
- `assetType` — тип вложения для Markdown
- `assetMarkdown` — готовый блок Markdown для вставки в документ ||
|| **select**
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/note.file.field.get`
...
- cURL (Webhook)
    ...
    -d '{"name":"assetMarkdown","select":["name","type","title"]}' \
    https://**put_your_bitrix24_address**/rest/api/**put_your_user_id_here**/**put_your_webhook_here**/note.file.field.get
    ```
...
- cURL (OAuth)
    ...
    -H "Content-Type: application/json" \
    -H "Accept: application/json" \
    -d '{"name":"assetMarkdown","select":["name","type","title"],"auth":"**put_access_token_here**"}' \
    https://**put_your_bitrix24_address**/rest/api/note.file.field.get
    ```
...
- Python
    ...
    try:
        bitrix_response = client.note.file.field.get(
            name='assetMarkdown',
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "note.file.field.get", b24.Params{
    	"name":   "assetMarkdown",
    	"select": []string{"name", "type", "title"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("note.file.field.get: %w", err)
    }
