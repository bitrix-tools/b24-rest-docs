# Получить список полей базы знаний note.collection.field.list
...
Метод `note.collection.field.list` возвращает список доступных полей базы знаний.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/note.collection.field.list`
...
- cURL (Webhook)
    ...
    https://**put_your_bitrix24_address**/rest/api/**put_your_user_id_here**/**put_your_webhook_here**/note.collection.field.list
    ```
...
- cURL (OAuth)
    ...
    https://**put_your_bitrix24_address**/rest/api/note.collection.field.list
    ```
...
- JS (TS)
    ...
    type CollectionFieldListResult = {
      items: CollectionFieldItem[]
    }

    try {
      const response = await $b24.actions.v3.call.make<CollectionFieldListResult>({
      ...
      if (!response.isSuccess) {
        ...
        console.info('Collection fields:', result.items)
      }
    } catch (error) {
      console.error(error)
    }
...
- JS (UMD)
    ...
    <script>
      async function listCollectionFields() {
        try {
      ...
      document.addEventListener('DOMContentLoaded', listCollectionFields)
    </script>
    ```
...
- Python
    ...
    try:
        bitrix_response = client.note.collection.field.list(
            select=select,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "note.collection.field.list", b24.Params{
    	"select": []string{"name", "type", "title", "filterable", "sortable"},
    }, b24.WithIdempotent())
    if err != nil {
    	return fmt.Errorf("note.collection.field.list: %w", err)
    }
...
- [{#T}](./note-collection-field-get.md)
- [{#T}](./note-collection-get.md)
- [{#T}](./index.md)
