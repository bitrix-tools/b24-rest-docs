# Архивировать базу знаний note.collection.archive
...
Метод `note.collection.archive` архивирует базу знаний и каскадно архивирует все ее документы.
...
`https://{адрес_установки}/rest/api/{id_пользователя}/{токен_вебхука}/note.collection.archive`
...
- Python
    ...
    try:
        bitrix_response = client.note.collection.archive(
            bitrix_id=42,
        ).response
...
- Go
    ...
    res, err := client.Core().Call(ctx, "note.collection.archive", b24.Params{
    	"id": 42,
    })
    if err != nil {
    	return fmt.Errorf("note.collection.archive: %w", err)
    }
...
`тип` | **Описание** ||
|| **result**
[`object`](../../data-types.md) | Объект с результатом архивации базы знаний ||
|| **result**
[`boolean`](../../data-types.md) | Значение `true`, если база знаний успешно архивирована ||
|| **time**
...
#|
|| **Поле** | **Описание ошибки** | **Как исправить** ||
|| `id` | База знаний не найдена | Проверьте, что база знаний существует и еще не архивирована ||
|#
