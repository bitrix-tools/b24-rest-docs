...
В Битрикс24 Вайбкод есть три типа ключей.
...
-  `vibe_live_...` — менеджмент-ключ для администрирования. Он не привязан к одному Битрикс24 и не обращается к данным — сделкам, задачам и другим объектам. Таким ключом управляют API-ключами, смотрят список подключенных Битрикс24 и работают с обращениями в поддержку платформы.

Для первого теста можно использовать личный ключ. Для приложения, которым будет пользоваться команда и которое открывается внутри Битрикс24, выбирайте ключ авторизации с OAuth — в мастере создания это режим «Приложение с авторизацией». Как опубликовать такое приложение — в разделе [Публикация приложения в каталоге Битрикс24](vibecode-tools.md#catalog).

### Режим доступа ключа {#access-mode}

У каждого ключа есть режим доступа — он не зависит от прав и определяет, может ли ключ менять данные.

-  `READWRITE` — чтение и запись, значение по умолчанию для новых ключей.

-  `READONLY` — только чтение. Запись отвечает ошибкой `403 WRITE_BLOCKED_READONLY_KEY`.

Действующий режим приходит в ответе `GET /v1/me` в поле `accessMode`, а переключают его в карточке ключа. Режим для новых ключей задает администратор в настройках Вайбкод: на уже выпущенные ключи эта настройка не влияет.
...
## Права на создание ключей и приложений {#rights}

Настройки Вайбкод определяют, кто создает ключи, приложения, серверы, ботов и агентов. Выпуск входящего вебхука и установку приложения дополнительно ограничивают настройки безопасности Битрикс24. Проверки идут по очереди: сначала Вайбкод, затем Битрикс24.

Какую настройку менять, видно по коду ошибки: `KEY_CREATION_RESTRICTED` и `APP_CREATION_RESTRICTED` приходят от Вайбкод, `CONNECTOR_KEY_ISSUE_FORBIDDEN` и `CONNECTOR_APP_INSTALL_FORBIDDEN` — от Битрикс24. Если вы не администратор, отправьте заявку на доступ из кабинета Вайбкод.

{% note tip "" %}

Режимы настроек, полный список кодов отказа и порядок проверок — в документации Вайбкод [Права на создание ключей и приложений](https://vibecode.bitrix24.tech/docs/access-rights).
...
Для типовых задач — дашборда, AI-бота, калькулятора — в кабинете Вайбкод есть раздел *Чертежи приложений* с готовыми техническими заданиями. Вы выбираете чертеж и копируете промпт, а агент скачивает техническое задание тем же ключом и строит приложение по нему. Раздел открывают постепенно, поэтому в кабинете его может пока не быть.
