...
-  `bitrixgpt-5` — BitrixGPT 5,

-  `bitrixgpt-5-vl` — BitrixGPT 5 Vision,
...
Для платных моделей можно использовать платформенный роутер Вайбкод: приложение обращается к единому API, а оплата считается по токенам.
...
[OpenClaw](https://vibecode.bitrix24.tech/ai-agents) — готовый AI-агент для Битрикс24, который работает как бот в чатах Битрикс24. Агент может получать доступ к данным Битрикс24 и выполнять действия через чат. Сотрудники могут писать агенту как коллеге: попросить найти сделку, создать задачу, подготовить сводку или проверить данные в CRM.

Возможности OpenClaw зависят от выданных прав. Агент может работать с CRM, задачами, документами, календарем и чатами только в рамках разрешенного доступа. Действия агента сохраняются в журнале, поэтому администратор может проверить, какие запросы выполнялись и какие данные использовались.

Для клиентов Битрикс24 AI-модель и подключение к Битрикс24 в OpenClaw бесплатные. Есть два сценария использования OpenClaw:

1. Запустить OpenClaw через Вайбкод. Платформа создаст сервер Black Hole, подключит Битрикс24, AI-модель и бота в чатах. Пользователь выбирает имя агента и права доступа.

2. Подключить к Битрикс24 свой OpenClaw. Вариант подходит, если агент уже развернут на сервере пользователя. Для подключения используется npm-плагин `@ihazz/bitrix24`, который регистрирует бота в Битрикс24, передает сообщения из чатов агенту и дает доступ к API Вайбкод в рамках прав ключа. Подробнее в статье [Подключение Битрикс24 к OpenClaw](https://vibecode.bitrix24.tech/docs/openclaw).

OpenClaw можно расширять навыками и подключать к инструментам Вайбкод: AI-моделям, AI-поиску и API для работы с данными Битрикс24.
...
По умолчанию для ключа действует лимит 60 запросов в минуту. Если приложение превысит лимит, Вайбкод вернет ошибку. В ответе будет текущий лимит, остатки запросов и время сброса. В этом случае приложение должно подождать и повторить запрос позже.
...
Параметры сервера зависят от выбранного тарифа: CPU, RAM, диск, регион и стоимость доступны при выборе сервера. По умолчанию действует лимит до 5 серверов на пользователя или API-ключ, если политика портала не задает другое значение. Если лимит превышен или создание серверов запрещено политикой портала, Вайбкод вернет ошибку.

**Как обновлять приложение?**

Опишите изменения AI-агенту. Агент может изменить код, проверить сценарий и повторно выполнить деплой через Вайбкод. После обновления проверьте права, данные, ошибки API и основной сценарий работы приложения.
