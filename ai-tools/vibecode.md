...
Модуль «Интеграция с Битрикс24 Вайбкод» должен быть установлен только на одной копии портала. Если модуль установлен на нескольких копиях Битрикс24, работающих с одним и тем же лицензионным ключом, это приведет к возникновению ошибок.

{% endnote %}

Как подключить Битрикс24 Вайбкод в коробочной версии:

1. Убедитесь, что на портале установлены все доступные обновления. Среди них должен присутствовать модуль `vibecodeconnector`.

2. Перейдите на страницу *Настройки > Настройки продукта > Модули*, найдите модуль `vibecodeconnector` и установите его, если установка не была выполнена автоматически.

3. Перейдите в настройки модуля «Интеграция с Битрикс24 Вайбкод».

4. Выполните привязку Вайбкод-сервера:

   -  убедитесь, что в поле «Добавить Битрикс24 Вайбкод-сервер» указано значение `https://vibecode.bitrix24.tech`,

   -  нажмите кнопку «Зарегистрировать».

5. Установите флажки «Видимость на портале» и «Открывать приложения внутри Битрикс24», затем сохраните изменения.

6. Перейдите в публичную часть портала, откройте Мессенджер и нажмите на появившийся значок каталога в левом верхнем углу.

7. Выберите пункт «Начать». После этого вы будете перенаправлены на сайт `https://vibecode.bitrix24.tech`. На странице выбора портала нажмите «Подключить коробочную версию Битрикс24».

8. Следуйте дальнейшим инструкциям мастера подключения.
...
Если у пользователя есть ключ OpenAI, Anthropic или другого OpenAI-совместимого провайдера, его можно подключить по сценарию BYOK. В этом случае запросы идут через Вайбкод, а оплата остается на стороне выбранного провайдера.
...
[Коворк/Код](https://vibecode.bitrix24.tech/cowork-code) — десктопное приложение для macOS и Windows с автономным AI-агентом для работы с Битрикс24. Агент ведет сделки, ставит задачи и собирает аналитику прямо в Битрикс24, а в режиме Код пишет и публикует приложения. Перед изменением или удалением данных агент показывает действие и запрашивает подтверждение.

Приложение, режим Код и AI-агенты работают по единой подписке: одна квота на AI-запросы для трех инструментов и один баланс. Квота считается в трех окнах — пять часов, неделя и месяц — и показывается в процентах. Бесплатный тариф доступен сразу, под рост задач подключаются тарифы Start, Pro и Max. Множитель тарифа применяется к цене и к объему квоты.
...
-  [Коворк/Код](https://vibecode.bitrix24.tech/docs/cowork) — единая подписка и состояние квоты на AI
