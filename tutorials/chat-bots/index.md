...
2. В разделе **Приложения → Разработчикам → Другое → Локальное приложение** создайте приложение и включите опцию «Использует только API»
3. В полях «Путь обработчика» и «Путь для первоначальной установки» укажите один и тот же адрес обработчика
4. Выдайте приложению права [`imbot`](../../api-reference/scopes/permissions.md) и [`task`](../../api-reference/scopes/permissions.md)
5. Сохраните приложение и скопируйте его `client_id` и `client_secret`
