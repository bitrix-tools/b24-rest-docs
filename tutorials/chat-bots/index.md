...
Наш чат-бот будет знать и обрабатывать только одну команду — «Что горит?», возвращая список просроченных задач. По аналогии всегда можно расширить его функционал, чтобы получать прямо в чат нужные отчеты на основании любых данных в _Битрикс24_.
...
- Выполните настройки. Вверху укажите название бота (у нас — «Докладун»). И затем выберите тип приложения — **Серверное**:
...
Если бот создается для коробочной версии _Битрикс24_ (не отмечена опция `Использует только API`), то используйте тип приложения **Статичное**, указав в обязательном поле `Архив с вашим приложением (zip)` путь к файлу-инсталлеру.
