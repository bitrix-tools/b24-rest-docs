...
Пример автоматической генерирации карточки редактирования компании со всеми полями, созданными в Битрикс24, на странице вашего приложения.
...
- JS
    ...
    app.get('/', async (req, res) => {
        ...
        arResult.FIELDS = (await $b24.actions.v2.call.make({
            method: 'crm.company.fields', params: {}, requestId: 'company-fields',
        })).getData().result
        ...
        if (id > 0) {
            arResult.ITEM = (await $b24.actions.v2.call.make({
                method: 'crm.company.get', params: { id }, requestId: 'item',
            })).getData().result
            // Связанные контакты компании (только их идентификаторы)
            arResult.VALUE_CONTACT_ID = (await $b24.actions.v2.call.make({
                method: 'crm.company.contact.items.get', params: { id }, requestId: 'contacts',
            })).getData().result
        ...
        for (const [key, field] of Object.entries(arResult.FIELDS)) {
            ...
            switch (field.type) {
                ...
                case 'crm_contact': {
                    ret = input({ ...params, TYPE: 'text' })
                    // Связи дают только ID — имена подтягиваем через crm.contact.list
                    const ids = (arResult.VALUE_CONTACT_ID ?? []).map((c) => c.CONTACT_ID)
                    if (ids.length) {
                        const contacts = (await $b24.actions.v2.call.make({
                            method: 'crm.contact.list', params: { filter: { ID: ids }, select: ['ID', 'NAME', 'LAST_NAME'] }, requestId: `ct-${key}`,
                        })).getData().result
                        if (contacts.length) {
                            ret += '(' + contacts.map((c) => [c.NAME, c.LAST_NAME].join(' ')).join(', ') + ')'
                        }
                    }
                    break
                }
...
- PHP
    ...
    <?php
        ...
        $arResult = [];
        $arResult['FIELDS'] = $crm->company()->fields()->getFieldsDescription();
        ...
        {//get item and standard enum field values if is update form
            ...
            $arResult['ITEM'] = iterator_to_array($crm->company()->get($ID)->company()->getIterator());
            // Связанные контакты компании (связи дают только идентификаторы)
            $arResult['VALUE_CONTACT_ID'] = [];
            foreach ($crm->companyContact()->get($ID)->getContactConnections() as $connection)
            {
                $arResult['VALUE_CONTACT_ID'][] = ['CONTACT_ID' => $connection->CONTACT_ID];
            }
        ...
        foreach ($arResult['FIELDS'] as $key => $arField)
        {
            ...
            switch ($arField['type'])
            {
                ...
                case 'crm_lead':
                    ...
                    break;
                case 'crm_contact':
                    // Связи дают только идентификаторы — имена подтянем через crm.contact.list
                    $arContact = [];
                    $arContactIds = !empty($arResult['VALUE_' . $key]) ? array_column($arResult['VALUE_' . $key], 'CONTACT_ID') : (($value) ? (array)$value : []);
                    if (!empty($arContactIds))
                    {
                        foreach ($crm->contact()->list([], ['ID' => $arContactIds], ['ID', 'NAME', 'LAST_NAME'], 0)->getContacts() as $contact)
                        {
...
- Python
    ...
    def form_page():
        ...
        ar_result = {}
        ar_result["FIELDS"] = client.crm.company.fields().result
        ...
        if item_id > 0:
            ar_result["ITEM"] = client.crm.company.get(bitrix_id=item_id).result
            # Связанные контакты компании (только их идентификаторы)
            ar_result["VALUE_CONTACT_ID"] = client.crm.company.contact.items.get(bitrix_id=item_id).result
        ...
        for key, field in ar_result["FIELDS"].items():
            ...
            if field_type == "crm_status":
                ...
                ret = input_field({**params, "TYPE": "text"})
                # Связи дают только ID — имена подтягиваем через crm.contact.list
                ids = [c["CONTACT_ID"] for c in ar_result.get("VALUE_CONTACT_ID", [])]
                if ids:
                    contacts = client.crm.contact.list(
                        filter={"ID": ids}, select=["ID", "NAME", "LAST_NAME"]).result
                    if contacts:
...
- JS
    ...
    export async function handler(req, res) {
        ...
        const arFields = (await $b24.actions.v2.call.make({
            method: 'crm.company.fields', params: {}, requestId: 'company-fields-save',
        })).getData().result
        ...
        if (id > 0) {
            const result = await $b24.actions.v2.call.make({
                method: 'crm.company.update', params: { id, fields: arForm }, requestId: 'company-update',
            })
            res.json({ message: result.getData().result ? 'Company update' : 'Company not updated' })
        } else {
            const result = await $b24.actions.v2.call.make({
                method: 'crm.company.add', params: { fields: arForm }, requestId: 'company-add',
            })
            const newId = result.getData().result
            res.json({ message: newId ? `Company add ID:${newId}` : 'Company not added' })
        }
    }
    ```
...
- PHP
    ...
    <?php
        ...
        $arFields = $crm->company()->fields()->getFieldsDescription();
        if(!empty($arFields))
        {
        ...
        if($arForm['ID'] > 0)
        {
            $id = $arForm['ID'];
            unset($arForm['ID']);
            $result = $crm->company()->update($id, $arForm)->isSuccess();
            $arMess = [
        ...
        {
            unset($arForm['ID']);
            $resultId = $crm->company()->add($arForm)->getId();
            $arMess = [
                'success' => 'Company add',
...
- Python
    ...
    def save_form():
        ...
        ar_fields = client.crm.company.fields().result
        ...
        ar_form.pop("ID", None)
        if item_id > 0:
            result = client.crm.company.update(bitrix_id=item_id, fields=ar_form).result
            message = "Company update" if result else "Company not updated"
        else:
            new_id = client.crm.company.add(fields=ar_form).result
            message = f"Company add ID:{new_id}" if new_id else "Company not added"
