...
- JS
    ...
    // Страница с формой: получаем описание полей и значения справочников, рендерим форму
    app.get('/', async (req, res) => {
        const id = parseInt(String(req.query.ID ?? '0'), 10) || 0

        // Описание полей и значения стандартных списков (последовательные вызовы)
        const arResult = {}
        arResult.FIELDS = (await $b24.actions.v2.call.make({
            method: 'crm.lead.fields', params: {}, requestId: 'lead-fields',
        })).getData().result
        ...
        arResult.FIELD_VALUES_STATUS_ID = (await $b24.actions.v2.call.make({
            method: 'crm.status.list', params: { filter: { ENTITY_ID: arResult.FIELDS.STATUS_ID.statusType } }, requestId: 'sts',
        })).getData().result
        ...
        if (id > 0) {
            // Получаем элемент и значения связанных объектов, если это форма редактирования
            arResult.ITEM = (await $b24.actions.v2.call.make({
                method: 'crm.lead.get', params: { id }, requestId: 'item',
            })).getData().result
            if (arResult.ITEM.COMPANY_ID && arResult.ITEM.COMPANY_ID !== '0') {
                arResult.VALUE_COMPANY_ID = (await $b24.actions.v2.call.make({
                    method: 'crm.company.get', params: { id: arResult.ITEM.COMPANY_ID }, requestId: 'company',
                })).getData().result
            }
            if (arResult.ITEM.CONTACT_ID && arResult.ITEM.CONTACT_ID !== '0') {
                arResult.VALUE_CONTACT_ID = (await $b24.actions.v2.call.make({
                    method: 'crm.contact.get', params: { id: arResult.ITEM.CONTACT_ID }, requestId: 'contact',
                })).getData().result
            }
        }

        // Рендер: по типу каждого поля строим подходящий input/select
        let sResult = ''
        let sResultCustom = ''
        for (const [key, field] of Object.entries(arResult.FIELDS)) {
            ...
            switch (field.type) {
                ...
                case 'crm_company':
                    ...
                    if (arResult.VALUE_COMPANY_ID && String(value) === String(arResult.VALUE_COMPANY_ID.ID)) {
                        ret += `(${arResult.VALUE_COMPANY_ID.TITLE})`
                    }
                    break
                case 'crm_contact':
                    ret = input({ ...params, TYPE: 'text' })
                    if (arResult.VALUE_CONTACT_ID && String(value) === String(arResult.VALUE_CONTACT_ID.ID)) {
                        ret += `(${[arResult.VALUE_CONTACT_ID.NAME, arResult.VALUE_CONTACT_ID.LAST_NAME].join(' ')})`
                    }
                    break
                case 'date':
...
- PHP
    ...
    <?php
        ...
        $arResult = [];
        $arResult['FIELDS'] = $crm->lead()->fields()->getFieldsDescription();
        ...
        $arResult['FIELD_VALUES_STATUS_ID'] = [];
        foreach($crm->status()->list([], ['ENTITY_ID' => $arResult['FIELDS']['STATUS_ID']['statusType']], [])->getStatuses() as $status)
        {
            $arResult['FIELD_VALUES_STATUS_ID'][$status->STATUS_ID] = $status->NAME;
        }
        ...
        if($ID > 0)//get item and standard enum field values if is update form
        {
            ...
            // доступ к значениям идёт по именам из crm.lead.fields.
            $arResult['ITEM'] = iterator_to_array($crm->lead()->get($ID)->lead()->getIterator());
            if(!empty($arResult['ITEM']['COMPANY_ID']))
            {
                $company = $crm->company()->get((int)$arResult['ITEM']['COMPANY_ID'])->company();
                $arResult['VALUE_COMPANY_ID'] = ['ID' => $company->ID, 'TITLE' => $company->TITLE];
            }
            if(!empty($arResult['ITEM']['CONTACT_ID']))
            {
                $contact = $crm->contact()->get((int)$arResult['ITEM']['CONTACT_ID'])->contact();
                $arResult['VALUE_CONTACT_ID'] = ['ID' => $contact->ID, 'NAME' => $contact->NAME, 'LAST_NAME' => $contact->LAST_NAME];
            }
        }
...
- Python
    ...
    def form_page():
        ...
        # Описание полей и значения стандартных списков (последовательные вызовы)
        ar_result = {}
        ar_result["FIELDS"] = client.crm.lead.fields().result
        ar_result["FIELD_VALUES_SOURCE_ID"] = client.crm.status.list(
            filter={"ENTITY_ID": ar_result["FIELDS"]["SOURCE_ID"]["statusType"]}).result
        ar_result["FIELD_VALUES_STATUS_ID"] = client.crm.status.list(
            filter={"ENTITY_ID": ar_result["FIELDS"]["STATUS_ID"]["statusType"]}).result
        ar_result["FIELD_VALUES_CURRENCY"] = client.crm.currency.list().result

        if item_id > 0:
            # Получаем элемент и значения связанных объектов, если это форма редактирования
            ar_result["ITEM"] = client.crm.lead.get(bitrix_id=item_id).result
            if ar_result["ITEM"].get("COMPANY_ID") and ar_result["ITEM"]["COMPANY_ID"] != "0":
                ar_result["VALUE_COMPANY_ID"] = client.crm.company.get(
                    bitrix_id=int(ar_result["ITEM"]["COMPANY_ID"])).result
            if ar_result["ITEM"].get("CONTACT_ID") and ar_result["ITEM"]["CONTACT_ID"] != "0":
                ar_result["VALUE_CONTACT_ID"] = client.crm.contact.get(
                    bitrix_id=int(ar_result["ITEM"]["CONTACT_ID"])).result
        ...
        for key, field in ar_result["FIELDS"].items():
            ...
            if field_type == "crm_status":
                ...
                company = ar_result.get("VALUE_COMPANY_ID")
                if company and str(value) == str(company["ID"]):
                    ret += f"({company['TITLE']})"
            elif field_type == "crm_contact":
                ret = input_field({**params, "TYPE": "text"})
                contact = ar_result.get("VALUE_CONTACT_ID")
                if contact and str(value) == str(contact["ID"]):
                    ret += f"({contact['NAME']} {contact['LAST_NAME']})"
...
- JS
    ...
    export async function handler(req, res) {
        ...
        const arFields = (await $b24.actions.v2.call.make({
            method: 'crm.lead.fields', params: {}, requestId: 'lead-fields-save',
        })).getData().result
        ...
        if (id > 0) {
            const result = await $b24.actions.v2.call.make({
                method: 'crm.lead.update', params: { id, fields: arForm }, requestId: 'lead-update',
            })
            res.json({ message: result.getData().result ? 'Lead update' : 'Lead not updated' })
        } else {
            const result = await $b24.actions.v2.call.make({
                method: 'crm.lead.add', params: { fields: arForm }, requestId: 'lead-add',
            })
            const newId = result.getData().result
            res.json({ message: newId ? `Lead add ID:${newId}` : 'Lead not added' })
        }
    }
    ```
...
- PHP
    ...
    <?php
        ...
        $arFields = $crm->lead()->fields()->getFieldsDescription();
        if(!empty($arFields))
        {
        ...
        if($arForm['ID'] > 0)
        {
            $id = $arForm['ID'];
            unset($arForm['ID']);
            $result = $crm->lead()->update($id, $arForm)->isSuccess();
            $arMess = [
                'success' => 'Lead update',
        ...
        {
            unset($arForm['ID']);
            $resultId = $crm->lead()->add($arForm)->getId();
            $arMess = [
                'success' => 'Lead add',
...
- Python
    ...
    def save_form():
        # request.form содержит ключи вида form[FIELD]; собираем вложенный словарь
        ar_form = {}
        ...
        # Приводим значения к формату REST по типу поля
        ar_fields = client.crm.lead.fields().result
        ...
        ar_form.pop("ID", None)
        if item_id > 0:
            result = client.crm.lead.update(bitrix_id=item_id, fields=ar_form).result
            message = "Lead update" if result else "Lead not updated"
        else:
            new_id = client.crm.lead.add(fields=ar_form).result
            message = f"Lead add ID:{new_id}" if new_id else "Lead not added"
