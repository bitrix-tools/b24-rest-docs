...
- JS
    ...
    app.get('/', async (req, res) => {
        ...
        arResult.FIELDS = (await $b24.actions.v2.call.make({
            method: 'crm.deal.fields', params: {}, requestId: 'deal-fields',
        })).getData().result
        ...
        if (arResult.FIELDS.STAGE_ID) {
            arResult.FIELD_VALUES_STAGE_ID = (await $b24.actions.v2.call.make({
                method: 'crm.status.list', params: { filter: { ENTITY_ID: arResult.FIELDS.STAGE_ID.statusType } }, requestId: 'stage',
            })).getData().result
        }
        ...
        if (id > 0) {
            arResult.ITEM = (await $b24.actions.v2.call.make({
                method: 'crm.deal.get', params: { id }, requestId: 'item',
            })).getData().result
            // Воронки (направления) сделок — для поля CATEGORY_ID
            arResult.VALUE_CATEGORY_ID = (await $b24.actions.v2.call.make({
                method: 'crm.dealcategory.list', params: { filter: { IS_LOCKED: 'N' } }, requestId: 'categories',
            })).getData().result
            // Предложения сделки — для read-only поля QUOTE_ID
            arResult.VALUE_QUOTE_ID = (await $b24.actions.v2.call.make({
                method: 'crm.quote.list', params: { filter: { DEAL_ID: id }, select: ['ID', 'TITLE'] }, requestId: 'quotes',
            })).getData().result
        ...
        let sResult = ''
        let sResultCustom = ''
        // CONTACT_ID/CONTACT_IDS устарели — используйте crm.deal.contact.items.get
        delete arResult.FIELDS.CONTACT_ID
        delete arResult.FIELDS.CONTACT_IDS
        for (const [key, field] of Object.entries(arResult.FIELDS)) {
            ...
            switch (field.type) {
                ...
                case 'crm_multifield':
                    ...
                    break
                case 'crm_category': {
                    const cats = Object.fromEntries((arResult.VALUE_CATEGORY_ID ?? []).map((c) => [c.ID, c.NAME]))
                    ret = select(params, cats)
                    break
                }
                case 'crm_quote':
                    // только для QUOTE_ID — read-only, показываем названия предложений
                    if ((arResult.VALUE_QUOTE_ID ?? []).length) {
                        ret += (arResult.VALUE_QUOTE_ID).map((q) => q.TITLE).join(', ')
                    }
                    break
                case 'crm_lead':
                ...
                case 'crm_company': {
                    ...
                    if (value) {
                        const companies = (await $b24.actions.v2.call.make({
                            method: 'crm.company.list', params: { filter: { ID: value }, select: ['ID', 'TITLE'] }, requestId: `co-${key}`,
                        })).getData().result
                        if (companies.length) ret += '(' + companies.map((c) => c.TITLE).join(', ') + ')'
                    }
                    break
                }
                case 'crm_contact': {
                    ret = input({ ...params, TYPE: 'text' })
                    if (value) {
                        const contacts = (await $b24.actions.v2.call.make({
                            method: 'crm.contact.list', params: { filter: { ID: value }, select: ['ID', 'NAME', 'LAST_NAME'] }, requestId: `ct-${key}`,
                        })).getData().result
                        if (contacts.length) ret += '(' + contacts.map((c) => [c.NAME, c.LAST_NAME].join(' ')).join(', ') + ')'
                    }
                    break
                }
                case 'date':
...
- PHP
    ...
    <?php
        ...
        $arResult = [];
        $arResult['FIELDS'] = $crm->deal()->fields()->getFieldsDescription();
        ...
        $arResult['FIELD_VALUES_STAGE_ID'] = [];
        if (isset($arResult['FIELDS']['STAGE_ID']))
        {
            foreach ($crm->status()->list([], ['ENTITY_ID' => $arResult['FIELDS']['STAGE_ID']['statusType']], [])->getStatuses() as $status)
            {
                $arResult['FIELD_VALUES_STAGE_ID'][$status->STATUS_ID] = $status->NAME;
            }
        }
        ...
        {//get item and standard enum field values if is update form
            ...
            $arResult['ITEM'] = iterator_to_array($crm->deal()->get($ID)->deal()->getIterator());
            // Воронки (направления) сделок — для поля CATEGORY_ID
            $arResult['VALUE_CATEGORY_ID'] = [];
            foreach ($crm->dealCategory()->list([], ['IS_LOCKED' => 'N'], [], 0)->getDealCategories() as $category)
            {
                $arResult['VALUE_CATEGORY_ID'][] = ['ID' => $category->ID, 'NAME' => $category->NAME];
            }
            // Предложения сделки — для read-only поля QUOTE_ID
            $arResult['VALUE_QUOTE_ID'] = [];
            foreach ($crm->quote()->list([], ['DEAL_ID' => $ID], ['ID', 'TITLE'], 0)->getQuotes() as $quote)
            {
                $arResult['VALUE_QUOTE_ID'][] = ['TITLE' => $quote->TITLE];
            }
        ...
        foreach ($arResult['FIELDS'] as $key => $arField)
        {
            ...
            switch ($arField['type'])
            {
                ...
                case 'crm_category':
                    ...
                    {
                        foreach ($arResult['VALUE_' . $key] as $category)
                        {
                            $arList[$category['ID']] = $category['NAME'];
                        }
                    }
                    $return = CPrintForm::select(
                        [
                ...
                case 'crm_lead':
                    ...
                    break;
                case 'crm_company':
                    // Связь даёт идентификатор — название подтянем через crm.company.list
                    $arCompany = [];
                    if (!empty($value))
                    {
                        foreach ($crm->company()->list([], ['ID' => $value], ['ID', 'TITLE'], 0)->getCompanies() as $company)
                        {
                            $arCompany[] = $company->TITLE;
                        }
                    }
                    $return = CPrintForm::input(
                        [
                    ...
                    if (!empty($arCompany))
                    {
                        $return .= '(' . implode(', ', $arCompany) . ')';
                    }
                    break;
                case 'crm_contact':
                    // Связь даёт идентификатор — имена подтянем через crm.contact.list
                    $arContact = [];
                    if (!empty($value))
                    {
                        foreach ($crm->contact()->list([], ['ID' => $value], ['ID', 'NAME', 'LAST_NAME'], 0)->getContacts() as $contact)
                        {
                            $arContact[] = implode(' ', [$contact->NAME, $contact->LAST_NAME]);
                        }
                    }
                    $return = CPrintForm::input(
                        [
...
- Python
    ...
    def form_page():
        ...
        ar_result = {}
        ar_result["FIELDS"] = client.crm.deal.fields().result
        ...
        if "STAGE_ID" in ar_result["FIELDS"]:
            ar_result["FIELD_VALUES_STAGE_ID"] = client.crm.status.list(
                filter={"ENTITY_ID": ar_result["FIELDS"]["STAGE_ID"]["statusType"]}).result
        ...
        # CONTACT_ID/CONTACT_IDS устарели — используйте crm.deal.contact.items.get
        ar_result["FIELDS"].pop("CONTACT_ID", None)
        ar_result["FIELDS"].pop("CONTACT_IDS", None)
        ...
        for key, field in ar_result["FIELDS"].items():
            ...
            if field_type == "crm_status":
                ...
                cats = {c["ID"]: c["NAME"] for c in ar_result.get("VALUE_CATEGORY_ID", [])}
                ret = select_field(params, cats)
            elif field_type == "crm_quote":
                # только для QUOTE_ID — read-only, показываем названия предложений
                quotes = ar_result.get("VALUE_QUOTE_ID", [])
                if quotes:
                    ret += ", ".join(q["TITLE"] for q in quotes)
                ...
                ret = input_field({**params, "TYPE": "text"})
                if value:
                    companies = client.crm.company.list(
                        filter={"ID": value}, select=["ID", "TITLE"]).result
                    if companies:
                        ret += "(" + ", ".join(c["TITLE"] for c in companies) + ")"
            elif field_type == "crm_contact":
                ret = input_field({**params, "TYPE": "text"})
                if value:
                    contacts = client.crm.contact.list(
                        filter={"ID": value}, select=["ID", "NAME", "LAST_NAME"]).result
                    if contacts:
                        ret += "(" + ", ".join(f"{c['NAME']} {c['LAST_NAME']}" for c in contacts) + ")"
...
- JS
    ...
    export async function handler(req, res) {
        ...
        const arFields = (await $b24.actions.v2.call.make({
            method: 'crm.deal.fields', params: {}, requestId: 'deal-fields-save',
        })).getData().result
        ...
        if (id > 0) {
            const result = await $b24.actions.v2.call.make({
                method: 'crm.deal.update', params: { id, fields: arForm }, requestId: 'deal-update',
            })
            res.json({ message: result.getData().result ? 'Deal update' : 'Deal not updated' })
        } else {
            const result = await $b24.actions.v2.call.make({
                method: 'crm.deal.add', params: { fields: arForm }, requestId: 'deal-add',
            })
            const newId = result.getData().result
            res.json({ message: newId ? `Deal add ID:${newId}` : 'Deal not added' })
        }
    }
    ```
...
- PHP
    ...
    <?php
        ...
        $arFields = $crm->deal()->fields()->getFieldsDescription();
        if(!empty($arFields))
        {
        ...
        if($arForm['ID'] > 0)
        {
            $id = $arForm['ID'];
            unset($arForm['ID']);
            $result = $crm->deal()->update($id, $arForm)->isSuccess();
            $arMess = [
                'success' => 'Deal update',
        ...
        {
            unset($arForm['ID']);
            $resultId = $crm->deal()->add($arForm)->getId();
            $arMess = [
                'success' => 'Deal add',
...
- Python
    ...
    def save_form():
        ...
        ar_fields = client.crm.deal.fields().result
        ...
        ar_form.pop("ID", None)
        if item_id > 0:
            result = client.crm.deal.update(bitrix_id=item_id, fields=ar_form).result
            message = "Deal update" if result else "Deal not updated"
        else:
            new_id = client.crm.deal.add(fields=ar_form).result
            message = f"Deal add ID:{new_id}" if new_id else "Deal not added"
