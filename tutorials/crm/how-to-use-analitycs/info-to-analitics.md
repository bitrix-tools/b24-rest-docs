...
1. Выбрать, достаточно ли UTM-полей или нужен полный трейс
2. Получить строку `TRACE` через `b24Tracker.guest.getTrace()`, если нужно передать полный маршрут клиента
3. Передать `TRACE` при создании объекта, если метод создания поддерживает это поле, или привязать трейс к готовым объектам методом [crm.tracking.trace.add](../../../api-reference/crm/tracking/crm-tracking-trace-add.md)
4. Сохранить идентификатор трейса из `result`, если его нужно удалить позже методом [crm.tracking.trace.delete](../../../api-reference/crm/tracking/crm-tracking-trace-delete.md)

## Что нужно до начала

- входящий вебхук или локальное приложение со scope [`crm`](../../../api-reference/scopes/permissions.md)
- права пользователя на создание или изменение объекта CRM, к которому нужно передать данные сквозной аналитики
- скрипт сквозной аналитики Битрикс24 установлен на страницах сайта, где собирается маршрут клиента
- вызовы REST выполняются на стороне сервера, если используете вебхук: путь вебхука нельзя раскрывать в браузере или публичном репозитории
- идентификаторы уже созданных объектов CRM, если трейс нужно привязать методом [crm.tracking.trace.add](../../../api-reference/crm/tracking/crm-tracking-trace-add.md)
...
Сохраните значение `result`. В примере это `341` — идентификатор, который нужно передать в параметр `id` метода [crm.tracking.trace.delete](../../../api-reference/crm/tracking/crm-tracking-trace-delete.md), если трейс нужно удалить.
...
При успешном удалении метод вернет `null` в поле `result`.
...
Проверка зависит от выбранного способа передачи данных.

#|
|| **Способ** | **Как проверить** ||
|| UTM-поля при создании объекта | Откройте созданный объект CRM и проверьте, что UTM-поля заполнены значениями из формы или интеграции ||
|| `TRACE` при создании объекта | Откройте созданный объект CRM и проверьте поле «Сквозная аналитика» ||
|| [crm.tracking.trace.add](../../../api-reference/crm/tracking/crm-tracking-trace-add.md) | Убедитесь, что метод вернул числовой `result`. Затем откройте объекты из `ENTITIES` и проверьте поле «Сквозная аналитика» ||
|| [crm.tracking.trace.delete](../../../api-reference/crm/tracking/crm-tracking-trace-delete.md) | Убедитесь, что метод вернул `result: null`, а в поле «Сквозная аналитика» связанного объекта значение очищено ||
|#

Если объект создан, но трейс не привязан, сценарий завершился частично. Проверьте объект перед повторным запуском, чтобы не создать дубликат.

## Обработка ошибок

Метод [crm.tracking.trace.add](../../../api-reference/crm/tracking/crm-tracking-trace-add.md) возвращает код `ERROR_CORE` в поле `error`, если не прошла проверка параметров `TRACE`, `ENTITIES` или прав на объекты. Конкретная причина приходит в поле `error_description`.

| `error_description` | Причина | Что проверить | С какого шага повторить |
|---|---|---|---|
| ``Parameter `TRACE` required.`` | Не передан трейс | Загрузку скрипта сквозной аналитики, вызов `b24Tracker.guest.getTrace()` и передачу значения на сервер | С шага 2 |
| ``Can not parse JSON in parameter `TRACE`.`` | `TRACE` не является корректной JSON-строкой | Что в `TRACE` передан результат `b24Tracker.guest.getTrace()` без изменений формата | С шага 2 |
| ``Wrong TYPE in parameter `ENTITIES`. Allowed types: COMPANY,CONTACT,DEAL,LEAD,QUOTE`` | Передан недопустимый тип объекта | Значения `TYPE`: доступны `COMPANY`, `CONTACT`, `DEAL`, `LEAD`, `QUOTE` | С шага 3 |
| ``Wrong ID in parameter `ENTITIES`.`` | Передан пустой, нечисловой или неположительный идентификатор объекта | Идентификаторы объектов из ответов методов создания или получения CRM | С шага 3 |
| ``You have no access to entity `CONTACT` with ID `123`.`` | Нет права изменить объект из `ENTITIES` | Права пользователя, от имени которого выполняется REST-вызов | После настройки прав повторите шаг 3 |

В последнем сообщении `CONTACT` и `123` приведены для примера. Метод подставляет фактические тип и идентификатор объекта.

Метод [crm.tracking.trace.delete](../../../api-reference/crm/tracking/crm-tracking-trace-delete.md) вернет ошибку `ERROR_CORE` с описанием ``Parameter `id` required.``, если не передать идентификатор трейса или передать пустое значение. Проверьте, что `id` взят из поля `result` метода [crm.tracking.trace.add](../../../api-reference/crm/tracking/crm-tracking-trace-add.md), и повторите удаление.

Системные REST-ошибки авторизации, прав и лимитов могут иметь другие коды. Если REST-вызов вернул ошибку, не повторяйте весь сценарий сразу: сначала проверьте, какие объекты уже созданы и был ли создан трейс.
