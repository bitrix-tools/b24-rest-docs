...
|| `200` | `ERROR_BATCH_METHOD_NOT_ALLOWED` | Method is not allowed for batch usage | Метод нельзя вызывать внутри `batch`: это загрузка или скачивание файла либо вложенный `batch` ||
|| `200` | `ERROR_BATCH_LENGTH_EXCEEDED` | Max batch length exceeded | В пакет передано больше 50 подзапросов. Первые 50 выполняются, каждый лишний подзапрос возвращает эту ошибку ||
|#
