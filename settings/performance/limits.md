...
При превышении общего времени выполнения запросов сверх 480 секунд в рамках прошедших 10 минут данный метод блокируется для приложения или веб-хука, превысившего лимит, на 10 минут. Приложение в ответ получает статус `429` с кодом ошибки `OPERATION_TIME_LIMIT`. При этом другие приложения и все остальные методы продолжают работать.
...
Однако есть [ряд рекомендаций](./index.md), следуя которым, вы можете снизить риски срабатывания ограничений по ресурсоемкости.
