...
Укажите параметр `URL_SETTINGS` в поле **Настройки приложения** при регистрации приложения в кабинете разработчика, в разделе REST-only. Если параметр заполнен, после установки приложения пользователь увидит слайдер с формой настройки.

Если параметр `URL_SETTINGS` не заполнен, форма настройки не откроется и Битрикс24 не отправит запрос для получения конфигурации формы.

### Жизненный цикл формы

1. Пользователь устанавливает приложение.
2. Битрикс24 делает POST-запрос на `URL_SETTINGS` с параметром `event: OnAppSettingsInstall`.
3. Приложение возвращает JSON-конфигурацию формы.
4. Битрикс24 показывает форму в слайдере.
5. При `updateForm: true` Битрикс24 повторно запрашивает `URL_SETTINGS` с параметром `event: OnAppSettingsChange`.
...
Битрикс24 отправляет POST-запрос на `URL_SETTINGS` для получения конфигурации формы. Значения `OnAppSettingsInstall`, `OnAppSettingsDisplay` и `OnAppSettingsChange` не являются отдельными событиями. Это значения параметра `event`, по которым приложение определяет сценарий запроса.

В ответ на запрос приложение должно вернуть JSON-конфигурацию формы. Если ответ корректный, Битрикс24 покажет форму в интерфейсе.

### Сценарии вызова

В примерах объект `auth` сокращен.

`OnAppSettingsInstall` передается после установки приложения.
...
`OnAppSettingsDisplay` передается, когда пользователь вручную открывает настройки приложения из карточки приложения.
...
`OnAppSettingsChange` передается, когда пользователь изменяет значение поля с параметром `updateForm: true`. В запросе дополнительно передается объект `data` с текущими значениями полей.
...
`тип` | **Описание** ||
|| **event** 
`string` | Сценарий запроса:
- `OnAppSettingsInstall` — после установки приложения
- `OnAppSettingsDisplay` — при открытии настроек приложения
- `OnAppSettingsChange` — при изменении поля с `updateForm: true` ||
|| **data**
`object` | Текущие значения полей формы. Передается для `OnAppSettingsChange` ||
|| **auth**
